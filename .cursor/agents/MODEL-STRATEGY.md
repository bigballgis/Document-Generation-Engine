...
**Only Composer and Claude Fable slugs** are permitted for subagents in this repo. Do not pin
GPT, Codex, Opus, Sonnet, or other families — even when a task is code-heavy.

| Family | Slugs used here |
| --- | --- |
| **Claude Fable** | `claude-fable-5-thinking-xhigh`, `claude-fable-5-thinking-high` |
...
| --- | --- | --- |
| **Orchestration** | `claude-fable-5-thinking-xhigh` | Multi-step routing, pipeline enforcement, high-stakes governance |
| **Reasoning** | `claude-fable-5-thinking-high` | BDD specs, docs, plan sync, commit review |
...
| --- | --- | --- |
| `delivery-orchestrator` | `claude-fable-5-thinking-xhigh` | End-to-end routing; must not skip gates or mis-order pipeline |
| `architecture-reviewer` | `claude-fable-5-thinking-xhigh` | Read-only but high impact; ADR/module/permission drift is costly |
| `behavior-spec-author` | `claude-fable-5-thinking-high` | Given/When/Then clarity and requirement traceability |
| `plan-orchestrator` | `claude-fable-5-thinking-high` | Structured plan layer; single active phase invariant |
| `doc-keeper` | `claude-fable-5-thinking-high` | Source-of-truth reconciliation across many docs |
...
| `post-task-commit-review` | `claude-fable-5-thinking-high` | Pre-commit review gate; block on critical findings |
...
3. **Family lock** — new assignments must stay within Composer + Claude Fable; update this doc when adding agents.
4. **Plan / Max Mode** — some billing tiers restrict non-Composer subagents; enable Max Mode or
   usage-based billing if a pinned Fable slug is ignored.
5. **Team policy** — org admin may block specific models; fall back to allowed Composer/Fable slugs and update this table.
