...
5. **TDD inner loop** — `pnpm -C frontend test:watch` or `pnpm -C frontend test --run <path>`
   (note: plain `pnpm test` runs full coverage — slower; use targeted runs while iterating).
