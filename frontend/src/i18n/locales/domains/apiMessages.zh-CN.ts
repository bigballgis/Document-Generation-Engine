  ...
  apimgmt: {
      policyImpact: {
        ...
        defaultRouteChanged: '默认路由目标将变更。',
                outputFormatsNarrowed: '允许的输出格式将收窄；部分调用方可能开始失败。',
        outputModesNarrowed: '允许的输出模式将收窄；部分调用方可能开始失败。',
        adGroupsNarrowed: '授权 AD 组将收窄；部分调用方可能被拒绝。',
        batchLimitLowered: '批处理条数上限将降低；更大的批次可能被拒绝。',
