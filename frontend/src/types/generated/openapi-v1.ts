...
export interface paths {
    ...
    "/api/management/v1/templates/{templateId}/compute-expressions/validate": {
         ...
         *     Whitelist `SPELL_AMOUNT` forms (IBL-A3): `SPELL_AMOUNT(value)` is always CNY Chinese uppercase (locale-independent). Optional `SPELL_AMOUNT(value, currencyCode)` takes an ISO 4217 alphabetic code; spelling language comes from `context.locale` primary language. Required supported pairs this leaf: `(en, USD)` and `(zh, CNY)`. Unsupported (language, currency) pair, illegal/blank currency, or arity ∉ {1,2} → `VARIABLE_COMPUTE_FAILED` (no silent wrong-language fallback). Behavior SoT: docs/behavior/ibl-a3-amount-in-words.md (BDD-IBL-A3-001…012).
         *     Whitelist `FORMAT_DATE` forms (PQH-F8 / IBL F8 residual): unary `FORMAT_DATE(value)` or optional `FORMAT_DATE(value, zoneId)` where `zoneId` is an IANA ZoneId (e.g. `Asia/Shanghai`, `UTC`) — not a locale tag. Instant-like unary conversion uses documented **UTC**; binary uses the explicit zone. Date-only `yyyy-MM-dd` / `LocalDate` are zone-independent (binary zone unused). Locale MEDIUM display still follows `context.locale` only. Do not infer zone from locale. Behavior SoT: docs/behavior/pqh-f8-format-date-tz.md (BDD-PQH-F8-001…012). Traceability: BDD-CE-K03-024…025; ADR-0056; IBL-A2; IBL-A3; PQH-F8.
         */
    ...
    "/api/management/v1/templates/{templateId}/compute-expressions/evaluate": {
         ...
         *     Whitelist `SPELL_AMOUNT` forms (IBL-A3): unary `SPELL_AMOUNT(value)` is always CNY Chinese uppercase (ignores locale language). Binary `SPELL_AMOUNT(value, currencyCode)` uses ISO 4217 currency identity + `locale` / `context.locale` primary language for spelling. At least `(en, USD)` and `(zh, CNY)` must succeed; unsupported pair, illegal/ blank currency, or bad arity → `VARIABLE_COMPUTE_FAILED` (no silent wrong-language fallback). Behavior SoT: docs/behavior/ibl-a3-amount-in-words.md (BDD-IBL-A3-001…012).
         *     Whitelist `FORMAT_DATE` forms (PQH-F8 / IBL F8 residual): unary `FORMAT_DATE(value)` resolves calendar day then MEDIUM-formats with `locale` / `context.locale`. Instant / `Date` / ISO datetime-with-offset or `Z` → **UTC** calendar day when zone omitted; binary `FORMAT_DATE(value, zoneId)` converts via IANA ZoneId. Date-only inputs ignore zone. Blank/invalid zone, bad arity, or unparseable datetime → `VARIABLE_COMPUTE_FAILED` (no silent bank TZ; no ISO prefix-truncate). Second arg is never a locale tag. Behavior SoT: docs/behavior/pqh-f8-format-date-tz.md (BDD-PQH-F8-001…012). Traceability: BDD-CE-K03-026…027; ADR-0056; IBL-A2; IBL-A3; PQH-F8.
         */
...
export interface components {
    schemas: {
        ...
        /** @description CE-K03 author-time sample evaluation payload (`POST …/compute-expressions/evaluate`). IBL-A2: binary `FORMAT_AMOUNT` optional ISO 4217 currency; invalid/blank currency → `VARIABLE_COMPUTE_FAILED`. Behavior: docs/behavior/ibl-a2-format-amount-currency.md. IBL-A3: unary `SPELL_AMOUNT` = CNY Chinese always; binary `SPELL_AMOUNT` = ISO + locale language; unsupported pair / illegal currency / bad arity → `VARIABLE_COMPUTE_FAILED`. Behavior: docs/behavior/ibl-a3-amount-in-words.md. PQH-F8: unary `FORMAT_DATE` Instant-like → UTC; binary optional IANA `zoneId`; blank/invalid zone / bad arity / unparseable datetime → `VARIABLE_COMPUTE_FAILED`. Behavior: docs/behavior/pqh-f8-format-date-tz.md. */
        ComputeExpressionEvaluateRequest: {
            /** @description Optional variable key being previewed (for error context). */
            variableKey?: string;
            /** @description Whitelist compute DSL expression to evaluate. Examples: `FORMAT_AMOUNT(${principal})` (locale-default currency); `FORMAT_AMOUNT(${principal}, 'EUR')` (ISO 4217 + locale formatting); `SPELL_AMOUNT(${principal})` (CNY Chinese always); `SPELL_AMOUNT(${principal}, 'USD')` with locale `en-US` (en+USD amount-in-words); `FORMAT_DATE(${signDate})` (date-only preferred for letter as-of); `FORMAT_DATE(${eventAt}, 'Asia/Shanghai')` (IANA zone for Instant). Invalid/blank currency, unsupported SPELL pair, blank/invalid ZoneId, or bad arity on binary forms → `VARIABLE_COMPUTE_FAILED`. */
            expression: string;
            /** @description Sample variable map for preview evaluation. Must not include secrets; values are author-supplied fixtures only. */
            sampleVariables: {
                [key: string]: unknown;
            };
            /** @description Optional BCP-47 locale tag (default zh-CN for FORMAT_* / SPELL_AMOUNT). Controls number/grouping localization for `FORMAT_AMOUNT` and MEDIUM date display for `FORMAT_DATE`; currency identity on binary `FORMAT_AMOUNT` comes from the ISO 4217 argument, not from this field (IBL-A2). For binary `SPELL_AMOUNT`, primary language of this locale selects the speller with the ISO currency (IBL-A3); unary `SPELL_AMOUNT` ignores locale language (always CNY Chinese). Does **not** select Instant→calendar-day conversion zone (PQH-F8 — use optional `zoneId` arg; unary Instant-like defaults to UTC). Default `zh-CN` means binary USD requires an explicit `en` / `en-US` locale. */
            locale?: string;
        };
    ...
    responses: {
        ...
        /** @description Unified error envelope. Runtime rate-limit may also return HTTP 429 (`RATE_LIMIT_EXCEEDED`) or, when distributed coordination fails, HTTP 503 (`RATE_LIMIT_BACKEND_UNAVAILABLE`); both use `category=RUNTIME`. */
        ErrorResponse: {
            headers: {
                /** @description Seconds until retry is advised. Present on `RATE_LIMIT_EXCEEDED` (429); optional on `RATE_LIMIT_BACKEND_UNAVAILABLE` (503). */
                "Retry-After"?: number;
                [name: string]: unknown;
            };
            content: {
