...
const approvalTemplates = computed(() =>
  templatesStore.templates.filter((item) => item.lifecycleStatus === 'APPROVAL'),
)

const groupedApprovalTemplates = computed(() => {
  const grouped = new Map<string, TemplateSummary[]>()
  for (const template of approvalTemplates.value) {
...
<template>
  <div class="workbench-page">
    <header class="page-header">
      <h1>{{ t('workbench.approver.title') }}</h1>
      <p>{{ t('workbench.approver.description') }}</p>
    </header>
    ...
    <template v-else-if="groupedApprovalTemplates.length > 0">
      <section
        v-for="[groupCode, items] in groupedApprovalTemplates"
        :key="groupCode"
        class="group-section"
      >
    ...
    <el-empty v-else :description="t('workbench.approver.empty')" />
  </div>
</template>
