...
const TASK_ID = 'CDP-E2E-T02' as const

test.describe('CDP-E2E-T02 UIUX evidence — tester pass decision @1920 (BDD-CDP-TEST-001)', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    fixture = await prepareRetailTemplateInTesting(request, {
      externalId: `E2E-CDP-T02-UX-${Date.now().toString(36).toUpperCase()}`,
      name: `E2E CDP T02 UX ${Date.now().toString(36).toUpperCase()}`,
    })
  })
  ...
  test('capture TEST queue + pass decision dialog (REDBC + GREENBC queue)', async ({ page, request }) => {
    ...
    await captureCdpE2eDecisionScreenshot(page, TASK_ID, '01-dashboard-test-queue-redbc-1920x1080.png')

    await switchBrand(page, 'GREENBC')
    await expect(page.locator('#tasks-section')).toBeVisible()
    await captureCdpE2eDecisionScreenshot(page, TASK_ID, '02-dashboard-test-queue-greenbc-1920x1080.png')

    await switchBrand(page, 'REDBC')
    await testRow.getByRole('button', { name: /^open$/i }).click()
    await expect(page).toHaveURL(/tab=lifecycle|\/dev\//, { timeout: 15_000 })

    // Open pass dialog, capture, then complete (reuse functional helper path after capture).
    ...
    await captureCdpE2eDecisionScreenshot(page, TASK_ID, '03-pass-decision-dialog-redbc-1920x1080.png')
