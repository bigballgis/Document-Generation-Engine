...
- **SYS-NORM D1（2026-07-21，Confirmed）：** 上述 DocumentBrand / LegalEntity **管理面产品要求**由 [ADR-0071](../adr/template-lifecycle/0071-retire-document-brand-legal-entity-surfaces.md) **supersede / withdraw**（信头/logo/签章归 Letterhead/母版；保留 Legal holds；壳层主题仍 UI-only）。ADR-0065 历史 Decision 与 IBL-E4 交付证据保留；前进方向以 D1 为准（见 [system-normalization-program.md](../behavior/system-normalization-program.md)）。
- 后台治理与管理界面主语言确认为英文。
...
- `permissions` 采用统一抽象权限枚举；**v1 仅对 `output.format=PDF` 映射并应用到输出访问权限**（CE-C06）；DOCX 动态加密仍支持 `openPassword`，但 **不**将 `permissions` 映射为 DOCX 写保护/权限位。v1 权限枚举确认为 `ALLOW_PRINT`、`ALLOW_COPY`、`ALLOW_EDIT`、`ALLOW_ANNOTATE`、`ALLOW_FORM_FILL`；不支持的权限枚举值返回 `400 ENCRYPTION_PARAMETER_INVALID`。
- **DOCX + 非空 `permissions`（CE-C06 / BDD-CE-C06，2026-07-15 确认）：** 在结构校验通过（含 `ownerPassword` 必填、密码基线等）后，请求**成功继续**；成功路径发出警告码 `DOCX_PERMISSIONS_NOT_APPLIED`（`generation.warning.fidelity.docxPermissionsNotApplied`），经 `fidelityWarnings[]` 或 SYNC_STREAM 保真警告头表达；**不**因此返回 400。Apache POI DOCX write-protect **不在**本切片范围。完整 Given/When/Then：[ce-c06-docx-permissions-boundary.md](../behavior/ce-c06-docx-permissions-boundary.md)。
...
- **PD-6（2026-07-19 产品意图确认 / 2026-07-20 BDD `ready`；OUT of Wave E）：** 同一 regenerate 入口支持显式 opt-in 生产重发：`productionReissue=true` + 非空 `reason`（建议 max 500）时，仅 `GLOBAL_ADMIN` / 同组 `GROUP_ADMIN` 可跳过 SPECIMEN（`specimen=false`）；`AUDIT_ADMIN` 仅默认证件样件路径（opt-in → 403）；缺 reason → 400 `PRODUCTION_REISSUE_REASON_REQUIRED`。默认 regenerate / preview / test-generate **继续强制** SPECIMEN；runtime formal 无水印语义不变。审计须区分模式（`productionReissue` / `specimen` / `reason`）。无新 capability bit；无新 ADR；**不**翻转 #3b/#5a。完整 Given/When/Then：[pd6-true-non-specimen-reissue.md](../behavior/pd6-true-non-specimen-reissue.md) `BDD-PD6-001…018`。
...
- **CE-K06b（2026-07-15 确认）：** `qrBarcodeRef` DOCX writer — ZXing 从 `variables[referenceKey]` 生成 PNG 嵌入；`sizePx`（默认 128，32–512）与 `errorCorrection`（`L|M|Q|H`，默认 `M`）及 `format`（`QR_CODE` 默认 / `CODE_128`）可配；退出 writer-unsupported set（`attachmentListRef` 仍阻断）；金标 `09-qr-barcode` ACTIVE；缺失 payload / 非法配置 / 编码失败显式失败、禁止静默省略。规格：[ce-k06-rendering-fidelity.md](../behavior/ce-k06-rendering-fidelity.md) `BDD-CE-K06b-001…009`。
- **CE-K06c（2026-07-15 确认 / BDD `ready`）：** `attachmentListRef` → `variables[referenceKey]` 为 `string[]`（每项 = 编号列表段落文本）；空数组成功零段落；`null`/缺失 key → `attachmentListPayloadMissing`；非数组/非 string 元素 → `attachmentListPayloadInvalid`；退出 writer-unsupported（集合可空）；金标 `10-attachment-list` ACTIVE；PDF 页码 stamp 以发布锁定 `renderProfile.pdfPageNumberStampingEnabled` 为包级权威，全局 `pdf-page-number-stamping-enabled` 不得绕过。规格：[ce-k06-rendering-fidelity.md](../behavior/ce-k06-rendering-fidelity.md) §18–§20 `BDD-CE-K06c-001…008`。
...
- **IBL-E5（2026-07-20 确认 / BDD `ready` / [ADR-0066 Accepted](../adr/template-lifecycle/0066-effectivefrom-publish-and-bulk-repin.md)）：** 发布门禁 `CONTENT_MODULE_EFFECTIVE_NOT_STARTED` + 管理 API `POST /api/management/v1/content-module-references/bulk-repin`（必填 `dryRun`；组范围草稿钉扎批量改钉；审计含 dry-run）。复用 `authorTemplates`；无新角色；无 `SCHEDULED` 发布生命周期；`frontend_ui_in_scope=false`。impl **Done**（`688f9e58` / `20ead1ce`）；**不**翻转 #3b/#5a。
- **IBL-E6（2026-07-20 确认 / BDD `ready` / [ADR-0067 Accepted](../adr/template-lifecycle/0067-clause-nesting-module-graph-governance.md)）：** CM↔CM 嵌套图写路径治理；深度上限 **8**（同构 `ComputeDslLimits`）；环路 fail-closed（写 / publish-gate / render）；扩展 CE-G05 `GET …/where-used`（`referenceKind` DIRECT\|NESTED + `nestingDepth` + `nestingPathSummary`）；传递钉扎发布硬门禁（`CONTENT_MODULE_NESTING_UNPINNED`）；lifecycle impact 同闭包。OpenAPI/domain/indexes synced。无新角色；`frontend_ui_in_scope=false`。完整 Given/When/Then：[ibl-e6-clause-nesting-governance.md](../behavior/ibl-e6-clause-nesting-governance.md) `BDD-IBL-E6-001…018`。impl **Done**（#133；MAIN `dcc42c81` / tip `0e542c03` / `32b62136`）；**sole-active cleared**；**不**翻转 #3b/#5a；**不**激活 #134。
...
- **催办时限设置 IA（已确认，2026-07-21 / TM #153）：** 全局默认仅经全局管理员 **系统设置** 全页维护；组级覆盖仅经分组管理员在分组/团队表面的 **团队设置** 对话框维护；Dashboard Overview **不得**承载催办时限配置面板。API `GET`/`PUT /api/management/v1/collaboration-timeout-config` 不变。行为：[reminder-timing-settings-ia.md](../behavior/reminder-timing-settings-ia.md)（BDD-RT-IA-001…016）；产品导航：[catalog-navigation-ux.md](../product/catalog-navigation-ux.md)。
...
- **CE-G03（2026-07-15 确认 / BDD `ready`）：** 变量 Schema 可选 `piiCategory`（`NONE` 缺省；`PERSONAL_NAME` / `GOVERNMENT_ID` / `FINANCIAL_ACCOUNT` / `CONTACT` / `ADDRESS` / `OTHER_SENSITIVE`）。保存测试数据集时，若 PII 标记字段提供非空值，必须 `piiHandling=SYNTHETIC`（合成/脱敏声明）或 `piiHandling=EXPLICIT_SENSITIVE`（原因 + 二次确认 + 管理审计）；否则 422 fail-closed。关闭 [data-storage-view.md](../architecture/data-storage-view.md)「masking vs always synthetic」挂起问题。规格：[ce-g03-testdata-pii.md](../behavior/ce-g03-testdata-pii.md) `BDD-CE-G03-001…017`。
...
- **CE-E01（2026-07-16 确认 / BDD `ready`）：** 支持自包含导出包 **v2**（`template-export-bundle-v2-json`）：ZIP 内嵌钉扎母版 DOCX（`artifacts/master.docx`）+ JSON 清单（CE-K01 母版 revision 指纹 `master_revision_id`/`master_file_hash`、条款正文快照、render profile 快照、资产键清单；默认**不**嵌资产二进制、**不**含 secret/凭证/测试数据明文）。默认导出仍为 v1 以兼容既有客户端；`bundleVersion=2` 显式选择 v2。导入支持 **dry-run**（`dryRun=true`）：仅返回依赖预检报告（母版指纹、条款将物化/缺失、资产键缺失等），**零**业务表写入；`readyToCommit=false` 时提交导入 **422**（`IMPORT_DEPENDENCIES_UNSATISFIED`）且禁止半残状态；通过则单事务落地 `DRAFT`（可物化缺失条款快照为草稿模块；母版仍绑定目标 `masterId`，本片不从 DOCX 自动建母版）。权限与矩阵 §5 导出/导入一致；**管理端 UI / E2E 本片 out of scope**（API-first；Import dry-run UI 见 Wave 7）。非目标：CE-E02 资产库、CE-E03 全库导出、CE-O01 PDF/A。完整 Given/When/Then：[ce-e01-export-bundle-v2.md](../behavior/ce-e01-export-bundle-v2.md) `BDD-CE-E01-001…018`；钉扎上游：[ce-k01-release-bundle-pinning.md](../behavior/ce-k01-release-bundle-pinning.md)。
- **CE-E02（2026-07-16 确认 / BDD `ready`）+ ALGI 组隔离（2026-07-22 / TM #154 / BDD `ready`）：** **组作用域资产库管理面**（**已 supersede** 平台共享目录 E02-C12）：管理 API 上传 / 分页列表 / 停用；身份 `(groupCode, assetKey)`；逻辑绑定键语法仍为 `^[A-Za-z][A-Za-z0-9._-]{0,127}$`（禁止绑定嵌入 `groupCode/`）；物理 MinIO 键 `{groupCode}/{assetKey}`；`assetClass`=`IMAGE`\|`SEAL`\|`OTHER`；上传 **必须** `groupCode`；列表可选 `groupCode` 过滤（非 GLOBAL 结果 ∩ 授权组；未授权过滤 → 空页）；`SEAL` 上传仅 `GLOBAL_ADMIN` / `GROUP_ADMIN`（Wave 5；原 `TEMPLATE_APPROVER` 已吸收）且组内生效；停用仅管理员且目标含 `groupCode`；capability `manageAssetLibrary` + 路由 `route.asset-library-management` → `/library/assets`；停用/迁移 quarantine 后移除可解析对象使渲染 fail-closed；跨组 ACTIVE **不得**满足模板解析；停用已 `DISABLED` → **已确认**幂等 `200`；上传 `Idempotency-Key` **预留 / 不强制不生效**；迁移 **ALGI-M1** quarantine-disable + admin rehome（禁止静默 ACTIVE→`PLATFORM`/`CORP`）。管理端 ScopedGroupSelect + E2E/UIUX in scope（ALGI）。非目标：Binding editor 重排、Auto `referenceKey`、CE-E03 全库导出、CE-O01 PDF/A、密码学电子签章、病毒扫描、跨组共享 v1。完整 Given/When/Then：[asset-library-group-isolation.md](../behavior/asset-library-group-isolation.md) `BDD-ALGI-001…018`；历史基线 + §15：[ce-e02-asset-library.md](../behavior/ce-e02-asset-library.md)；契约：[openapi-v1.yaml](../api/openapi-v1.yaml)。
- **CE-E03（2026-07-17 确认 / BDD `ready`）：** 支持 **全库导出** ZIP（`template-library-export-v1-zip`）：根 `library-export-manifest.json` + 嵌套每个合格模板的 CE-E01 v2 自包含 ZIP（`templates/{templateId}.zip`）+ 去重母版目录 `masters/{masterFileHash}.docx` + 去重条款快照目录 `clauses/…`；聚合 `assetKeyManifest`（**仅键**，默认不嵌资产二进制）。默认范围 = 调用方授权 ∩ 导出合格生命周期；可选 `groupId` / `templateIds` 收窄；部分成功（单模板 FAILED 不阻断其它）；空集/超限（500）→ 422。权限与矩阵 §5「导出模板」一致（无新权限码）。**全库导入 / 管理端 UI / E2E 本片 out of scope**（API-first，同 E01）。非目标：CE-G05、CE-O02、CD-3、go-live、#50。完整 Given/When/Then：[ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md) `BDD-CE-E03-001…017`；上游：[ce-e01-export-bundle-v2.md](../behavior/ce-e01-export-bundle-v2.md)。
- **SYS-NORM Wave 7 — UAT→PROD 晋级包（2026-07-21 确认 / BDD `ready`）：** 在 CE-E01 v2 / CE-E03 之上提供 opt-in 晋级依赖闭合参数 `dependencyClosure=PROMOTION`：单模板 ZIP 嵌入 `artifacts/assets/{assetKey}` 二进制 + 条款嵌套闭包（`clauseNestingGraph`，深度≤8）；全库导出时嵌套包同为晋级包且根级可去重 `assets/`。**禁止** DocumentBrand/LegalEntity sidecar 与 secrets。导入沿用 `POST …/templates/import`（`dryRun` 零写入；提交落地模板 **DRAFT**；可选物化 CE-E02 资产与 **DRAFT** 母版；**禁止**经 pack 跳过母版 APPROVED；须在生产重新测试→审批→发布）。管理端 Templates Import 对话框 dry-run UI in scope（**Check dependencies** → 报告 → Import 仅当 `readyToCommit=true`）。权限复用矩阵 §5 导出/导入（无新码）。默认非晋级 E01/E03 路径 fail-closed **不得回退**。非目标：Wave 8、parked UX、CE-O02、翻 #3b/#5a、标 #53 Done、宣称 SYS-NORM 程序 Done。完整 Given/When/Then：[sys-norm-promotion-pack.md](../behavior/sys-norm-promotion-pack.md) `BDD-SYS-NORM-PP-001…020`；契约：[openapi-v1.yaml](../api/openapi-v1.yaml) / [contract-outline.md](../api/contract-outline.md)；决策复用 [ADR-0071](../adr/template-lifecycle/0071-retire-document-brand-legal-entity-surfaces.md) Decision 5。
...
> 用户确认（2026-07-08）：当前生成的演示文档**内容存在但 Word 排版/版式/字体/样式不达标**，不符合国际大型银行对外信函品质；Word 为最佳排版工具，演示输出须达到**外资银行信函级**观感。实现计划：[P23-demo-typography-layout-excellence.md](../plan/detail/P23-demo-typography-layout-excellence.md)（**Done** 2026-07-08；T01–T16）；行为规格 [demo-typography-layout-behavior-spec.md](demo-typography-layout-behavior-spec.md)（**ready** — `BDD-DEMO-TYP-001`…`020`）。Task Master 任务 4–8 与本阶段对齐。
