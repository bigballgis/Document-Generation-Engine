...
- **CE-E03（2026-07-17 确认 / BDD `ready`）：** 支持 **全库导出** ZIP（`template-library-export-v1-zip`）：根 `library-export-manifest.json` + 嵌套每个合格模板的 CE-E01 v2 自包含 ZIP（`templates/{templateId}.zip`）+ 去重母版目录 `masters/{masterFileHash}.docx` + 去重条款快照目录 `clauses/…`；聚合 `assetKeyManifest`（**仅键**，不嵌资产二进制）。默认范围 = 调用方授权 ∩ 导出合格生命周期；可选 `groupId` / `templateIds` 收窄；部分成功（单模板 FAILED 不阻断其它）；空集/超限（500）→ 422。权限与矩阵 §5「导出模板」一致（无新权限码）。**全库导入 / 管理端 UI / E2E 本片 out of scope**（API-first，同 E01）。非目标：CE-G05、CE-O02、CD-3、go-live、#50。完整 Given/When/Then：[ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md) `BDD-CE-E03-001…017`；上游：[ce-e01-export-bundle-v2.md](../behavior/ce-e01-export-bundle-v2.md)。
