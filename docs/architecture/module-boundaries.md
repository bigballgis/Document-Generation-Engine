...
| Asset Library (CE-E02) | Group-scoped image/seal catalog (`library_asset` keyed by `groupCode` + `assetKey`), MinIO object keys for ACTIVE assets, management upload/list/disable APIs and admin UI route `/library/assets` | `StructuredContentImageResolver` protocol, authorization policy source of truth, generation orchestration, CE-E03 full-library export |
| Library Export (CE-E03) | Full-library ZIP export under `com.bank.docgen.library` (`POST …/library/export`, manifest + nested E01 v2 packs); reuses Template Composition export eligibility/access | Asset catalog CRUD (CE-E02), template authoring/lifecycle ownership, single-template export/import HTTP surface (CE-E01) |
