...
3. Active hygiene slice (Task Master **#49** / `deps-security-refresh`, 2026-07-13) — **confirmed session constraints** (ops hygiene; not product requirements):
   ...
   - Critical/High in changed dependency scope: remediate via baseline-safe bump **or** record an exception with the metadata in [Exception Handling](#exception-handling) (aligns with M9-T03 pattern: owner + expiry + cleanup task).
   ...
   - Behavior readiness: [deps-security-refresh.md](../behavior/deps-security-refresh.md) (`bdd_readiness: not-applicable`).
