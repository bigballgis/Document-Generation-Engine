...
| **D01A-C9** | **PDF 池有界（冻结 NFR 默认）：** `docgen.rendering.conversion-pool-size` 默认 **2**（`PDF_CONVERSION_POOL_SIZE`）；`conversion-queue-capacity` 默认 **0**（`PDF_CONVERSION_QUEUE_CAPACITY`）；core=max=poolSize；`RejectedExecutionHandler=AbortPolicy`。**禁止**将默认队列改为无界（如 `Integer.MAX_VALUE` / `LinkedBlockingQueue` 无界）作为本叶「修复」。运维可通过 env **有界**调大，但产品默认保持 fail-fast。 | NFR + SOR-P03 + F4-C5 |
