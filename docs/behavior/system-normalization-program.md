...
| Field | Value |
| --- | --- |
| **文件状态** | Wave 0 charter **`ready`**（plan + ADR SoT）；Wave 1 BDD **ready/Done**；Wave 2 BDD **ready**（[sys-norm-hub-ia.md](./sys-norm-hub-ia.md)）；Waves 3–8 实现 BDD = **stub / pending-wave** |
...
| **Placement** | ISOLATED |
| **Task / leaf** | Wave 0 TM **#143** `sys-norm-charter` → **Done**；Wave 1 TM **#145** `sys-norm-shell-fluid-nav` → **Done**（handoff briefly cited `#144` — `#144` remains PTA）；Wave 2 TM **#146** `sys-norm-hub-ia` → BDD **ready** (impl Not Started); Waves 3–8 **Not Started** in program plan |
...
per_wave_impl_bdd: Wave 1 ready/Done; Wave 2 ready ([sys-norm-hub-ia.md](./sys-norm-hub-ia.md)); Waves 3–8 stubs / pending-wave
open_questions:
...
| Per-wave runtime BDD (§8) | Wave 1 **ready/Done**; Wave 2 **ready** (dedicated file); Waves 3–8 **stub / pending-wave** | Author full G/W/T at wave start — do not invent ready scenarios here for unstarted waves |
...
| 2 | `sys-norm-hub-ia` | **ready** (TM **#146**; [sys-norm-hub-ia.md](./sys-norm-hub-ia.md) **BDD-SYS-NORM-W2-001…018**) | Template (+ Master parity) Hub Properties drawer; remove wrong tabs; version Dependencies; API jump model A; Dev blank-surface; locale de-dupe; legacy apiAccess |
