...
| **G05-C13a（IBL-E6 扩展指针）** | **深度/嵌套 where-used（非 CE-G05 原交付）：** IBL-E6 / [ibl-e6-clause-nesting-governance.md](./ibl-e6-clause-nesting-governance.md) 扩展同端点：权威=CM↔CM 嵌套图 + 祖先钉扎（非全量 binding JSON 扫描）；行增 `referenceKind` DIRECT\|NESTED、`nestingDepth`、`nestingPathSummary`。G05 直接钉扎语义保留；写路径环/深度与传递钉扎门禁见 E6-C* / [ADR-0067 Accepted](../adr/template-lifecycle/0067-clause-nesting-module-graph-governance.md)。 | F28 / IBL-E6 |
