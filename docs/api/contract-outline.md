...
**IBL-E4 / PD-9（BDD `ready`，2026-07-20；Task Master #131；[ADR-0065 Accepted](../adr/template-lifecycle/0065-legal-entity-document-brand-variants.md)）：** Runtime `context` 白名单新增可选 `legalEntityCode`。组范围管理面 DocumentBrand / LegalEntity 目录（list/create/update）+ 组 `defaultLegalEntityCode`；模板包可选 `allowedDocumentBrandCodes`。解析：法人 → 绑定 DocumentBrand；省略 legalEntity → 组默认实体品牌或种子 `PLATFORM_DEFAULT`；未知/INACTIVE → **422**（`LEGAL_ENTITY_UNKNOWN` / `LEGAL_ENTITY_INACTIVE` / `DOCUMENT_BRAND_INACTIVE`）；allow-list 不匹配 → **422** `DOCUMENT_BRAND_NOT_ALLOWED`。成功路径摘要回显 `resolvedLegalEntityCode` + `resolvedDocumentBrandCode`（非敏感）。路径仍钉扎模板版本；**不**按法人自动选包；DocumentBrand ≠ UI `REDBC`/`GREENBC`。管理 UI 必交付（`frontend_ui_in_scope=true`）。行为 SoT：[ibl-e4-entity-document-brands.md](../behavior/ibl-e4-entity-document-brands.md)（**BDD-IBL-E4-001…017** / E4-C*）。正式字段以 [OpenAPI v1](openapi-v1.yaml) 为准。Accepted ADR ≠ E4 impl Done；**不**翻转 #3b/#5a。
...
| `legalEntityCode` | 调用方法人实体码（可选）。 | 字符串；IBL-E4 文档品牌解析轴（ADR-0065）；trim；空串视为缺失；建议 max 64；大小写敏感 exact 匹配；**不**进入 inclusion；**不**选包。 |
