...
- **PQH-F8（IBL F8 / Q2 residual）：** 白名单 `FORMAT_DATE` 支持一元 `FORMAT_DATE(value)` 与可选二元 `FORMAT_DATE(value, zoneId)`（**IANA ZoneId**；第二参**不是** locale 标签）。Instant / `java.util.Date` / 带 offset 或 `Z` 的 ISO datetime：一元 → 文档化 **UTC** 日历日；二元 → 显式 zone。日期-only `yyyy-MM-dd` / `LocalDate` 与 zone 无关（二元 zone 可写但忽略）。**禁止**从 `context.locale` 推断时区；**不**新增 `context.timeZone`（ADR-0013 不变）。ISO datetime **不得**靠 `substring(0,10)` 截断成功。Locale 仍仅驱动 MEDIUM 展示（ADR-0056 Amendment 2026-07-23）。空白/非法 zone、非法 arity、不可解析 datetime → **422** `VARIABLE_COMPUTE_FAILED`（既有码）。OpenAPI：同上 compute validate/evaluate。行为 SoT：[pqh-f8-format-date-tz.md](../behavior/pqh-f8-format-date-tz.md)（BDD-PQH-F8-001…012）。程序：[post-queue-hardening-program-2026-07.md](../plan/post-queue-hardening-program-2026-07.md)。**不**翻转 checklist #3b/#5a；**不**宣称 IBL/go-live Done；**不**重开 IBL Wave A。
...
**PQH-F8（2026-07-23；Task Master #160）：** `FORMAT_DATE(value)` 或可选 `FORMAT_DATE(value, zoneId)`（**IANA**）；一元 Instant-like → **UTC**；二元空白/非法 zone、非法 arity、不可解析 datetime → 既有 **`VARIABLE_COMPUTE_FAILED`**（同上 HTTP/category/messageKey；禁止静默 bank TZ / ISO 前缀截断）。本叶**不**新增顶层错误码；**不**新增 `context.timeZone`。行为 SoT：[pqh-f8-format-date-tz.md](../behavior/pqh-f8-format-date-tz.md)（BDD-PQH-F8-001…012）。Formal phase **None**；**not** go-live；do **not** flip **#3b/#5a GO**；do **not** claim IBL program Done。
| `GENERATION` | `VARIABLE_COMPUTE_FAILED` | `api.error.variable.computeFailed` | `false` | Whitelist compute expression evaluation failed (incl. invalid FORMAT_AMOUNT currency; unsupported/illegal SPELL_AMOUNT pair; invalid FORMAT_DATE zoneId / datetime). |
