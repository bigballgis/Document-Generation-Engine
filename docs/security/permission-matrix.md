...
## 13. 登录起点角色旅程与路由可见性（T01 首波）

已确认：

- 首波角色旅程重构范围仅覆盖 `GLOBAL_ADMIN`、`GROUP_ADMIN`、`TEMPLATE_AUTHOR`。
- 本次登录鉴权边界维持现有本地认证假设，不新增 SSO/OIDC 范围。
- 迁移策略采用新旧导航并行并由特性开关控制，关闭开关时沿用既有导航权限行为。

角色到路由可见性矩阵（逻辑路由标识，非最终 URL）

| 逻辑路由标识 | GLOBAL_ADMIN | GROUP_ADMIN | TEMPLATE_AUTHOR | 权限基线说明 |
| --- | --- | --- | --- | --- |
| `route.global-governance-home` | 允许 | 禁止 | 禁止 | 全局治理起点。 |
| `route.group-governance-home` | 允许 | 允许（被授权组范围内） | 禁止 | 分组治理起点。 |
| `route.template-authoring-home` | 允许 | 允许（被授权组范围内） | 允许（所属或被授权组范围内） | 模板编排起点。 |
| `route.api-policy-management` | 允许 | 允许（被授权组范围内） | 禁止 | 与第 7 章 API 管理权限一致。 |
| `route.audit-console` | 允许 | 允许（被授权组范围内） | 禁止 | 与第 10 章审计查看权限一致。 |
...
- 逻辑路由标识对应的前端真实 URL 与命名最终稿。
- `TEMPLATE_AUTHOR` 与既有“母版设计人员”在路由级权限映射是否完全对齐。
- 无权访问反馈页面的按钮级授权策略（返回上一页/回首页/联系管理员）。
