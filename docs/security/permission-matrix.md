...
### 13.1.1 过渡路由与 Dashboard 合并（待 COR-T11 最终决策）
...
| `route.tester-workbench` | `/dashboard` | `TesterWorkbenchView` 仍存在；测试队列由 Dashboard 任务区承载 |
| `route.approver-workbench` | `/dashboard` | `ApproverWorkbenchView` 仍存在；审批队列由 Dashboard 任务区承载 |

**待决（COR-T11）：** 恢复分组 workbench 专用队列，或增强 Dashboard 后移除 dead code 与遗留视图。
