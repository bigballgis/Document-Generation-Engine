...
| **CDP** (CD-DOC/CD-BDD/CD-E2E/CD-HARD, `CD-*`) | **Active sibling focus** — Wave CD-2 **In Progress** (**partial** — T01–T09 Done; **T10 In Progress**; T11–T13 Not Started) — track via [CDP program](./competitiveness-deepening-program.md) | **Nothing in LRP.** Do not execute `CD-*` from LRP session. LRP references CD-2/CD-3 as sibling waves only. |
...
**Current wave:** none sole-active — **LR-A Done** (2026-07-10); **LR-B Done** (2026-07-04); **LR-C** remains partial (C1/C4 Done) when next scheduled. Word-vs-LO / ADR-0042 Accepted + ADR-0043 slice B **deferred out of Wave LR-A exit**; ADR-0041 Accepted residual **closed** (architecture-reviewer 2026-07-10). Program status: **In Progress** (other waves remain). **Sibling:** CDP Wave CD-2 **partial** (T01–T09 Done, merges `1930842` / `6821f45` / `895f16e` / `3aed175` / `c62b1a1` / `1eb230b` / `55a6ab6`; **T10 In Progress**; T11–T13 Not Started).
...
**Next action (2026-07-11):** **LR-C9 Done** — no LRP task currently `In Progress`. Formal phase remains **None**. Wave LR-A **Done** (A1–A7); Word/XSD residuals remain deferred. Wave LR-C partial (C1/C4/C9 Done; C2/C3/C5/C7/C8/C10–C13 **Not Started**). **Sibling CDP:** **CD-E2E-T10 In Progress** (slice `cdp-e2e-t10-fidelity`; T01–T09 Done; T11–T13 Not Started — do not execute from LRP; do **not** activate LR-C10 while T10 sole-active). **Recommended LRP candidates (after CDP T10 or when scheduled):** **LR-C10** (upload UX polish), **LR-C11** (i18n `api.error` closure), **LR-C12** (keyboard a11y). LR-B residual follow-ups: browser-level SSE incremental proof → **LR-E1**; JWT_SECRET default-fallback guard → **LR-E2** launch checklist prerequisite.
