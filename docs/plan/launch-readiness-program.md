...
**Program ID:** `LRP`  
**Created:** 2026-07-03  
**Status:** **Not Started** (planned 2026-07-03)  
...
**Current wave:** none — program **Not Started** (planned 2026-07-03).
...
| ID | Owner | Title | Depends | BDD | Maps | Status |
| --- | --- | --- | --- | --- | --- | --- |
| LR-B1 | doc-keeper + architecture-reviewer | Deployment topology decision ADR-0044 (single vs multi replica) | — | not-applicable | CD-PIT-14; ADR-0039; P15 HPA | Not Started |
| LR-B2 | backend-engineer | Scheduler distributed mutex (ShedLock JDBC or DB lock) | LR-B1 | not-applicable | CD-PIT-14; new dependency → policy check + ADR | Not Started |
| LR-B3 | backend-engineer + frontend-engineer | SSE production readiness (heartbeat, headers, nginx SSE location) | — | not-applicable (verified by LR-E1) | CD-PIT-12 | Not Started |
| LR-B4 | deploy-engineer + backend-engineer | Async transport production topology (Kafka or accepted in-process) | LR-B1 | not-applicable | Ledger seam «Async batch transport» | Not Started |
| LR-B5 | backend-engineer | Graceful shutdown & drain (server, pools, SSE) | — | not-applicable | §1 finding 7 | Not Started |
| LR-B6 | backend-engineer + frontend-engineer | Session renewal + revocation (no SSO/OIDC) | User session-policy confirmation | **required** | CD-PIT-13; COR-F03 | Not Started |
| LR-B7 | backend-engineer | Idempotency digest hard-fail + rate-limit filter fail-closed alignment | — | not-applicable | OPT-E9; COR-B10 residual; ADR-0031 | Not Started |
...
**Next action (when LRP activates):** confirm activation with the user → set **LR-B** `In Progress` in this file + ledger § LRP → delegate **LR-B1** to doc-keeper + architecture-reviewer, and **LR-B3/B5/B8** per detail doc. **Do not** start `P22-*` or `CD-*` from here.
