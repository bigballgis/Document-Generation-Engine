...
| **LRP** (`LR-*` — production hardening + usability deepening) | **This program** — Wave LR-A **Done** (2026-07-10); **Wave LR-C Done** (2026-07-11 — C1–C13; merge tip `bf9cbeb`); **Wave LR-D → In Progress** (partial — **LR-D1 Done** merge `20b2a76`; **LR-D2 Done** merge `3d78bc5`; **LR-D3 Done** merge `ba5ea2e`; **LR-D7 Done** merge `c94a356`; **LR-D6 Done** merge `56383eb`; **LR-D5 Done** merge `5b13476`; **LR-D4 In Progress** slice `lrp-d4-trace-propagation`; Task Master #41; **sole-active**); LR-E **Not Started** | LR-A…LR-E per §2 wave map; A1–A7 Done; Word/XSD/LO24/0042-Accepted/0043-Accepted residuals deferred post-wave (0041 Accepted residual closed). |
...
**Current wave:** **LR-D** (**In Progress** — partial; **sole-active = LR-D4**) — **LR-A Done** (2026-07-10); **LR-B Done** (2026-07-04); **Wave LR-C Done** (2026-07-11 — C1–C13; merge tip `bf9cbeb`). **LR-D4 → In Progress** (slice `lrp-d4-trace-propagation`; Task Master #41; BDD not-applicable confirming in parallel; ISOLATED `../DGE-lrp-d4-trace-propagation` · base `19457bf`). **LR-D3 → Done** (slice `lrp-d3-metrics-alerting`; merge `ba5ea2e`; feature tip `173ab36`; Task Master #40; BDD not-applicable; mvn verify GREEN 1315; architecture PASS_WITH_NOTES; DEPLOY_OK scrape 2026-07-12T03:43:07+08:00). **LR-D2 → Done** (slice `lrp-d2-backup-restore`; merge `3d78bc5`; Task Master #39; BDD not-applicable; drill PASS RPO≈0.933min RTO≈4.751min; healthz 200; FOL OK). **LR-D1 → Done** (slice `lrp-d1-audit-retention`; merge `20b2a76`; Task Master #35; BDD ready; ADR-0048 Accepted; V54). **LR-D7 → Done** (slice `lrp-d7-durable-security-audit`; merge `c94a356`; Task Master #36; BDD **`ready`**; seam «Security forbidden-route audit» **closed**). **LR-D6 → Done** (slice `lrp-d6-load-smoke`; merge `56383eb`; Task Master #37; BDD not-applicable; Scenario A n=20 success=12 errorRate=0.4 triaged DEF-LRP-D6-001; Scenario B 5/5 SSE dropped=0). **LR-D5 → Done** (slice `lrp-d5-nfr-proposals`; merge `5b13476`; Task Master #38; BDD not-applicable; docs-only NFR proposals — **pending confirmation, NOT confirmed SLOs**). Do **not** activate LR-E/CD-3. Do **not** touch `DGE-audit-governance`. Do **not** mark Wave LR-D Done. Word-vs-LO / ADR-0042 Accepted + ADR-0043 slice B **deferred out of Wave LR-A exit**; ADR-0041 Accepted residual **closed** (architecture-reviewer 2026-07-10). Program status: **In Progress**. Formal phase remains **None**. **Sibling:** CDP Wave CD-2 **Done** (T01–T13; merge tip `b2b0899`; **no sole-active CDP E2E slice**).
...
| LR-D4 | backend-engineer | Trace propagation decision + minimal impl (traceId → Kafka/async/MDC) | — | not-applicable | §1 finding 12 | **In Progress** (2026-07-12 — slice `lrp-d4-trace-propagation`; Task Master #41; **sole-active**; base `19457bf`) |
...
- **Status:** Not Started (checklist artifact not yet authored; **LR-D2 drill input ready** — do **not** treat Wave LR-D or launch as Done)
...
**Next action (2026-07-12):** **Sole-active = LR-D4** (slice `lrp-d4-trace-propagation`; Task Master #41). Formal phase remains **None**. **Wave LR-D → In Progress** (partial — **LR-D1 Done** merge `20b2a76`; **LR-D2 Done** merge `3d78bc5`; **LR-D3 Done** merge `ba5ea2e`; **LR-D7 Done** merge `c94a356`; **LR-D6 Done** merge `56383eb`; **LR-D5 Done** merge `5b13476`; **LR-D4 In Progress**). NFR proposals authored as pending — **not** confirmed SLOs. Do **not** activate LR-E/CD-3. Do **not** touch `DGE-audit-governance`. Do **not** mark Wave LR-D Done until D4 exit gate. Pipeline next: **backend-engineer** gap-close per **[ADR-0049 Accepted](../adr/operations/0049-distributed-trace-propagation.md)** (MDC decorator + Kafka `X-Trace-Id`; reuse ADR-0047 baseline; no Zipkin/Tempo). **Sibling CDP:** Wave CD-2 **Done** (T01–T13; merge tip `b2b0899`; **no sole-active CDP E2E slice** — do not execute from LRP). LR-B residual follow-ups (later): browser-level SSE incremental proof → **LR-E1**; JWT_SECRET default-fallback guard → **LR-E2** launch checklist prerequisite (backup/restore drill item now has dated evidence).
