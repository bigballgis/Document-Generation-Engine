...
| ID | Pri | Title | Evidence | Acceptance | Status |
| --- | --- | --- | --- | --- | --- |
| C1 | H | Test `apimgmt` module (currently 0 tests) | `ApiManagementService.java` (~326 L), controller, 17 classes no tests | Service + controller unit/slice tests; ≥85% on changed lines | Not Started |
| C2 | H | Test authorization core | `GroupAccessService.java`, `ManagementAuthService.java`, `JwtAuthenticationFilter.java` untested | RBAC/fail-closed paths tested incl. deny cases | Not Started |
