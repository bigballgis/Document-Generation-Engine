...
| SOR-P06 | Medium | Frontend bundle optimization | `frontend/src/main.ts:3-15` — full Element Plus global import + full CSS; no `manualChunks` in `vite.config.ts` | On-demand component imports + chunk strategy; list virtualization only where row count is unbounded | Done | **2026-07-04:** `unplugin-auto-import` + `unplugin-vue-components` (Element Plus on-demand sass); `build/manualChunks.ts`; base theme only in `main.ts`; list virtualization deferred (server paging landed in P01) |
...
| SOR-F03 | High | De-duplicate API policy UI + dedicated store | `TemplateDetailApiAccessTab.vue` (**691** lines) vs `ApiPolicyDetailView.vue` (**596** lines) duplicate the policy domain editor; policy/credentials state lives in the **549**-line `stores/templates.ts` god store | Shared `ApiPolicyDomainEditor`; dedicated apiPolicy store keyed by `templateId` | Done | **2026-07-04:** `stores/apiPolicy.ts` (**278** lines, **8** Vitest tests) keyed by `templateId`; shared `ApiPolicyDomainEditor.vue` (**703** lines, **3** tests) + `useApiPolicyDomainForms.ts`; `TemplateDetailApiAccessTab.vue` **691 → 142**; `ApiPolicyDetailView.vue` **596 → 405**; `templates.ts` **572 → 403**. BDD `not-applicable` (refactor only) |
| SOR-F04 | High | Consolidate direct API calls through stores/composables | 15+ components call API modules directly, bypassing store cache (e.g. `TemplateTestDataSetPanel.vue:89-222`, `TemplateCoveragePanel.vue:52`, `TemplateVersionLinesPanel.vue:71`) — stale-data risk after mutations | Data access via stores/composables with explicit invalidation rules | Done | **2026-07-04:** `stores/templatePanelData.ts` (**~400** lines, **6** Vitest tests) keyed by `templateId` with explicit invalidation (test data sets → coverage/eligibility; preview start → preview runs; version lines → release versions/change diff; content-module refs → coverage/change diff); **11** template panel components migrated (no direct `@/api/templates` in `components/templates/*.vue` or `BatchTestHistoryPanel.vue`). BDD `not-applicable` (refactor only) |
...
| SOR-F06 | Medium | Request cancellation + retryable-error UX | No `AbortController` usage anywhere; `errorEnvelope.ts:28-38` parses `retryable` but no retry affordance exists | Cancellation on route-leave; central retryable-error UX | Done | **2026-07-04:** `storeRequestSupport` + `useAbortableCatalogLoader`; catalog lists (users/masters/templates/content-modules/audit) abort + `LoadErrorPanel` retryable |
| SOR-F07 | Low | Split `UserManagementPanel.vue` | `frontend/src/views/identity/UserManagementPanel.vue` — **578**-line security-sensitive monolith | Split list / form / reset-password dialogs | Done | **2026-07-04:** `UserManagementListSection`, `UserFormDialog`, `UserResetPasswordDialog`, `userPasswordRules` |
...
| SOR-A02 | Medium | Rendering-facing ports/DTOs (decouple rendering from template/authorization) | `PreviewGenerationService` imports `TemplateService`/`TestDataSetService`; `AsyncBatchTestOrchestrator` imports `GroupAccessService` | Ports/DTOs at the rendering boundary; module-boundary doc updated | Blocked | **Coordinate with P22 session; do not start while P22 edits rendering** |
| SOR-A03 | Medium | Continue god-service split (OPT-D5 remaining) | `ApiManagementService` 535 L, `BatchGenerationService` 511 L, `TemplateLifecycleService` 544 L, `TemplateVersionLineService` 586 L, `DocxAssembler` 638 L | Focused services extracted per OPT-D5 pattern; `DocxAssembler` split only after P22 lands | Blocked | OPT-D5 (§10); P22 overlap |
...
| SOR-A06 | Medium | Distributed tracing | Micrometer Prometheus registry only; no OTel dependency | Micrometer Tracing + OTLP export; traceId propagated to audit consistently | Done | **2026-07-04:** [ADR-0047](../adr/operations/0047-distributed-tracing-otlp-baseline.md); `micrometer-tracing-bridge-otel` + `opentelemetry-exporter-otlp`; `TraceIdProvider` → Micrometer span → `X-Trace-Id` → UUID; tracing disabled local/test; prod OTLP via `OTEL_EXPORTER_OTLP_ENDPOINT`; `TraceIdProviderTest` (7 cases) |
...
| SOR-T02 | Medium | A11y depth beyond smoke | `e2e/a11y-smoke.spec.ts` checks headings/buttons only; no `@axe-core/playwright`; dialog focus management rarely tested | Axe scans on key views; dialog focus audit | Done | **2026-07-04:** `e2e/sor-a11y-axe.spec.ts` — axe WCAG2/2.1 AA scans + create-user dialog focus trap |
| SOR-T03 | Medium | Close E2E journey gaps + expand docker subset | Identity/group admin journeys, user CRUD, password reset, forbidden-page content untested; docker acceptance subset is 5/51 specs | New journeys + expanded tagged subset (with SOR-C03 tiering) | Done | **2026-07-04:** `e2e/sor-journey-gaps.spec.ts`; docker subset **8/51** specs (+ a11y-smoke, sor-a11y-axe, sor-journey-gaps) |
...
| M9-T02/T03 intranet SCA; M10–M11 security closure | [m9-task-sheet.md](../architecture/m9-task-sheet.md) and m10/m11 siblings | SOR-C05 SBOM wiring **feeds** M9-T02; Task Master **#49** (`deps-security-refresh`) **Done** (`08c7d56`) — in-repo remediation hygiene; **does not** close M9-T02 or reopen SOR-C05 |
