...
| SOR-F06 | Medium | Request cancellation + retryable-error UX | No `AbortController` usage anywhere; `errorEnvelope.ts:28-38` parses `retryable` but no retry affordance exists | Cancellation on route-leave; central retryable-error UX | In Progress | **2026-07-04 slice:** `useRouteScopedAbortController` wired to identity user list fetch + `lastUserErrorRetryable` + `LoadErrorPanel`; broader store/axios adoption deferred |
