...
| CE-G05 | 模板年检 + 条款正文检索 | P3·M · `In Progress` | `nextReviewDue` + 到期待办；`content_structure_json` 全文检索（PostgreSQL tsvector）where-used。**Sole-active (2026-07-17)** — Task Master **#77** **in-progress**；slice `ce-g05-annual-review-fts`；worktree `D:/working/DGE-ce-g05-annual-review-fts` · `feat/ce-g05-annual-review-fts`。BDD **ready** ([ce-g05-annual-review-fts.md](../behavior/ce-g05-annual-review-fts.md) `BDD-CE-G05-001…019`)；`frontend_ui_in_scope=true`（E2E/UIUX 必修）。Last remaining P3 after **#80** Done。Formal phase **None**；**not** go-live；**not** CD-3。 | 无 |
