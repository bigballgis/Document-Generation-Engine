...
| ID | 标题 | 级/量 | 要点 | 依赖 |
| --- | --- | --- | --- | --- |
| CE-O01 | PDF/A 输出选项 | P2·M · `In Progress` | render profile 加 `pdfArchivalProfile`（NONE/PDF_A_2B）；LibreOffice 过滤器参数切换；与加密互斥校验；小 ADR 记录选型；veraPDF 或最小自校验进金标。**In Progress (2026-07-16)** — Task Master **#81** sole-active; slice `ce-o01-pdfa-output`; worktree `DGE-ce-o01-pdfa-output` · `feat/ce-o01-pdfa-output`; BDD **ready** ([ce-o01-pdfa-output.md](../behavior/ce-o01-pdfa-output.md) `BDD-CE-O01-001…016`); [ADR-0058 Accepted](../adr/rendering-authoring/0058-pdfa-2b-archival-output.md)（D6=A-2b）；**FE/E2E out of scope**（API/render-only）。Formal phase **None**；**not** go-live；**not** CD-3。 | K07 |
...
| 序 | CE-ID | Task # | 级 | 说明 |
| --- | --- | --- | --- | --- |
| **1（sole-active）** | **CE-O01** | **#81** | P2 | PDF/A-2b 归档输出 — Task Master **in-progress**（deps K07 #54 + E02 #79 **Done**；BDD ready；ADR-0058 Accepted；worktree `DGE-ce-o01-pdfa-output` · `feat/ce-o01-pdfa-output`；FE/E2E out of scope） |
| **2（O01 后下一叶）** | **CE-G04** | **#75** | P2 | Legal hold 最小实现 — **pending**/parked（deps G03 #74 **Done**；P2 优先于 E03 P3；do **not** activate until O01 Done + deliver） |
| 3 | CE-U14 | #90 | P2 | Dashboard 模板生命周期待办 — **pending**/parked（deps U13 #89 **Done**；U15–U21 链跟其后） |
| 4+ | CE-E03 / U17+ / G05 | #80 / #96… / #77 | P3 | 全库导出、快捷键/依赖只读、年检 FTS — **parked**（P3；do **not** start another CE leaf while #81 In Progress） |

**已并入 main、勿再并行复开：** CE-E02 (#79) — merge `5bd3611e` on `main`（2026-07-16）；CE-E01 (#78) — merge `6ae57974` on `main`（2026-07-16）；CE-G06 (#76) — merge `d8636232` on `main`（2026-07-16）；CE-C06 (#71) — merge `35f6f47d` on `main`（2026-07-16）；CE-U13 (#89) — merge `ccdfacda` on `main`（2026-07-16）；CE-G03 (#74) — merge `50c1a524` on `main`（2026-07-16）；CE-K08 (#63) — `35f57503` on `main`。Task Master / 本文件状态应对齐 **Done**。 **Sole-active CE delivery leaf = #81 CE-O01**（umbrella **#53** is program registry, not a delivery leaf）。
...
**Task Master registry (2026-07-16):** umbrella **#53** in-progress (program registry only); **Execution model:** **single-lane serial** (§9.2) — **sole-active CE delivery leaf = #81 CE-O01** (`in-progress`; slice `ce-o01-pdfa-output`; worktree `DGE-ce-o01-pdfa-output`). **#79** CE-E02 → **Done** (merge `5bd3611e`) — do **not** reopen. **#78** CE-E01 → **Done** (merge `6ae57974`) — do **not** reopen. **#76** CE-G06 → **Done** (merge `d8636232`) — do **not** reopen. **#71** CE-C06 → **Done** (merge `35f6f47d`) — do **not** reopen. Do **not** start another CE leaf while #81 is In Progress. Leaves **#54–#97** (CE-O02 skipped per D5). **Batch 1–4 Done.** Prior waves closed: **#59**/**#68**/**#83** → **Done**; **#60**/**#84**/**#85** → **Done**. **Wave 0 (K05+U11+U12) closed:** **#61** CE-K05 + **#86** CE-U11 + **#87** CE-U12 → **Done** (merges `51a58f12` / `513d776c` / tip `34353b75`). **#69** CE-C04 → **Done** (`c7be8305`). **#70** CE-C05 → **Done** (`405f7cea`). **#88** CE-U06 → **Done** (`7734366e`). **#62** CE-K06 → **Done** (K06a `485a7f3e` + K06b `a689ca87` + K06c tip `76297d08`). **#63** CE-K08 → **Done** (`35f57503` on main). **#74** CE-G03 → **Done** (`50c1a524`). **#89** CE-U13 → **Done** (`ccdfacda`). Formal phase remains **None**. Do **not** activate CD-3. Do **not** claim go-live.

**Last reviewed:** 2026-07-16（#81 CE-O01 sole-active In Progress；O01 后下一叶 #75 CE-G04 parked；然后 #90 CE-U14；#79 CE-E02 Done；umbrella #53 in-progress；formal phase None；not go-live；not CD-3）
