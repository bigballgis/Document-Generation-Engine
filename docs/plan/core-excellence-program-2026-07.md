...
| CE-E03 | 全库导出 | P3·M · `In Progress` | 基于 per-template bundle + 母版/条款批量导出 + manifest。**In Progress (2026-07-17)** — Task Master **#80** sole-active；slice `ce-e03-full-library-export`；worktree `D:/working/DGE-ce-e03-full-library-export` · `feat/ce-e03-full-library-export`。**BDD `ready`** — [ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md) `BDD-CE-E03-001…017`；API-first（FE/E2E **out of scope**，同 E01）。**OpenAPI landed** — `POST …/library/export` (`exportLibraryTemplates`)；权限复用矩阵 §5 导出模板。Handoff: **backend-engineer**（TDD/API）。Formal phase **None**；**not** go-live；**not** CD-3；**not** CE-O02；leave **#50** alone。 | E01 |
