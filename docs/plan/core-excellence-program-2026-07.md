...
| CE-C05 | `originalBatchId` 重试血缘 | P1·M · `In Progress` | **In Progress (2026-07-15)** — Task Master **#70**; slice `ce-c05-original-batch-id`; worktree `DGE-ce-c05-original-batch-id` · `feat/ce-c05-original-batch-id`. BDD **ready** ([ce-c05-original-batch-id.md](../behavior/ce-c05-original-batch-id.md); **BDD-CE-C05-001…012**): request field + same-credential `BATCH_ROOT` validation + `ORIGINAL_BATCH_NOT_FOUND` + response echo + audit association + contract docs. Lane after C04 (`c7be8305`). Formal phase **None**; **not** Done; **not** go-live; gates pending (implementation). | C04（泳道） |
