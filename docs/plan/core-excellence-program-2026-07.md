...
| CE-C04 | 凭证 `expires_at` 持久化 + 暴露 | P1·M · `In Progress` | **In Progress (2026-07-15)** — Task Master **#69**; slice `ce-c04-credential-expires`; worktree `DGE-ce-c04-credential-expires` · `feat/ce-c04-credential-expires`. BDD **ready** ([ce-c04-credential-expires.md](../behavior/ce-c04-credential-expires.md); **BDD-CE-C04-001…012**). Wave 1 lane 3. End `ApiCredentialLifecycleSupport` transitional; Flyway `expires_at` + issue/rotate write; `RuntimeCredentialSummaryView` `expiresAt`/`EXPIRING_SOON`; optional CallableVersion `deprecated`/`sunsetAt`. Rebased onto MAIN `485a7f3e` (K06a). Formal phase **None**; **not** Done; **not** go-live. Gates pending. | 无 |
...
**Last reviewed:** 2026-07-15（Wave 1 parallel #62+#69 In Progress; K06a at 485a7f3e; Wave 0 Done; #88 pending）
