...
| CE-U21 | 草稿按锚点分 key + 并发提示 | P2·M · `Done` | **Done (2026-07-17)** — Task Master **#95**; slice `ce-u21-draft-anchor-concurrency`. BDD **ready** ([ce-u21-draft-anchor-concurrency.md](../behavior/ce-u21-draft-anchor-concurrency.md); **BDD-CE-U21-DAC-001…012**). **Merge:** `8d8f6f6d` / feature `292fbc35` on `main` (worktree removed). localDraft key 加 `anchorId`（legacy key 兼容）；binding upsert `expectedUpdatedAt` / 409 `bindingVersionConflict` + Reload/Keep editing UX. **Gates:** BE `mvn verify` **GREEN**; FE lint/type-check/test/build **GREEN**; Stage 5 **DEPLOY_OK** ([ce-u21-stage5-deploy](./evidence/ce-u21-stage5-deploy/)); E2E **4/4** ([CE-U21-manifest](../../frontend/e2e/evidence/CE-U21-manifest.md); [ce-u21-stage6-e2e](./evidence/ce-u21-stage6-e2e/)); UIUX **PASS** Critical=0 ([CE-U21-uiux-manifest](../../frontend/e2e/evidence/CE-U21-uiux-manifest.md)); architecture **Critical=0**; Stage 10 **DEPLOY_OK** ([ce-u21-stage10-deploy](./evidence/ce-u21-stage10-deploy/)). **P2 continuum complete** with U21. Formal phase **None**; **not** go-live；**not** CD-3。 |
...
**Last reviewed:** 2026-07-17（Stage 12 post-task-doc-sync — #95 CE-U21 → Done；merge `8d8f6f6d` / feature `292fbc35`；worktree removed；P2 continuum complete；no sole-active leaf；P3 parked；leave #50 alone；umbrella #53 in-progress registry only；formal phase None；not go-live；not CD-3；not activate P3）
