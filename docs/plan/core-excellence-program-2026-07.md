...
| CE-U18 | 批量测试历史钻取 + 双路径统一 | P2·M · `Not Started` | **§9.2 next serial** after U16 — Task Master **#93** **pending/parked**（do not auto-activate until stage 0）。`sampleResultsJson` 前端消费（展开明细+跳转）；移除无进度 legacy 同步批量路径 |
...
| 序 | CE-ID | Task # | 级 | 说明 |
| --- | --- | --- | --- | --- |
| **1（next / parked）** | **CE-U18** | **#93** | P2 | 批量测试历史钻取 — **pending**（deps U16 #92 **Done**；activate at stage 0） |
