...
| CE-G06 | 审计可复现最小集 | P2·M · `In Progress` | invocation 记录发布包快照 ID + bundle hash；管理端"按 invocation 受控再生"API（需权限 + 审计 + SPECIMEN 水印标记再生件）。**Sole-active (2026-07-16):** Task Master **#76** **in-progress**; slice `ce-g06-audit-reproducible`; worktree `D:/working/DGE-ce-g06-audit-reproducible` · `feat/ce-g06-audit-reproducible`; placement **ISOLATED**. BDD **ready** ([ce-g06-audit-reproducible.md](../behavior/ce-g06-audit-reproducible.md); `BDD-CE-G06-001…021`)；FE 再生 UI **out of scope**。Formal phase **None**；**not** go-live；do **not** reopen CE-C06 (#71). | K01 |
...
| 序 | CE-ID | Task # | 级 | 说明 |
| --- | --- | --- | --- | --- |
| **1** | **CE-G06** | **#76** | P2 | 审计可复现最小集 — **sole-active In Progress**（ISOLATED `DGE-ce-g06-audit-reproducible` · `feat/ce-g06-audit-reproducible`）；BDD ready |
| 2+ | 收敛期 | #78… / #90… | P2–P3 | E01 → E02 → O01；再 U14–U21 链 — **parked** until #76 merge + MAIN doc-sync |

**已并入 main、勿再并行复开：** CE-C06 (#71) — merge `35f6f47d` on `main`（2026-07-16）；CE-U13 (#89) — merge `ccdfacda` on `main`（2026-07-16）；CE-G03 (#74) — merge `50c1a524` on `main`（2026-07-16）；CE-K08 (#63) — `35f57503` on `main`。Task Master / 本文件状态应对齐 **Done**。 **Sole-active CE leaf:** **#76** CE-G06 only（umbrella **#53** is program registry, not a delivery leaf）。
...
**Task Master registry (2026-07-16):** umbrella **#53** in-progress (program registry only); **Execution model:** **single-lane serial** (§9.2) — **sole-active CE delivery leaf:** **#76** CE-G06 → **in-progress** (ISOLATED `D:/working/DGE-ce-g06-audit-reproducible` · `feat/ce-g06-audit-reproducible`; BDD ready). **#71** CE-C06 → **Done** (merge `35f6f47d`) — do **not** reopen. Leaves **#54–#97** (CE-O02 skipped per D5). **Batch 1–4 Done.** Prior waves closed: **#59**/**#68**/**#83** → **Done**; **#60**/**#84**/**#85** → **Done**. **Wave 0 (K05+U11+U12) closed:** **#61** CE-K05 + **#86** CE-U11 + **#87** CE-U12 → **Done** (merges `51a58f12` / `513d776c` / tip `34353b75`). **#69** CE-C04 → **Done** (`c7be8305`). **#70** CE-C05 → **Done** (`405f7cea`). **#88** CE-U06 → **Done** (`7734366e`). **#62** CE-K06 → **Done** (K06a `485a7f3e` + K06b `a689ca87` + K06c tip `76297d08`). **#63** CE-K08 → **Done** (`35f57503` on main). **#74** CE-G03 → **Done** (`50c1a524`). **#89** CE-U13 → **Done** (`ccdfacda`). Formal phase remains **None**. Do **not** activate CD-3. Do **not** claim go-live.

**Last reviewed:** 2026-07-16（#76 CE-G06 sole-active In Progress；umbrella #53 in-progress；#71 CE-C06 Done；formal phase None；not go-live）
