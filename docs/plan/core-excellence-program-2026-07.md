...
| CE-U19 | 依赖关系只读视图 | P3·M · `In Progress` | **In Progress (2026-07-17)** — Task Master **#97**; slice `ce-u19-dependency-readonly-view`. BDD **ready** ([ce-u19-dependency-readonly-view.md](../behavior/ce-u19-dependency-readonly-view.md); **BDD-CE-U19-DRV-001…012**). Placement **ISOLATED** `D:/working/DGE-ce-u19-dependency-readonly-view` · `feat/ce-u19-dependency-readonly-view`. Package Hub **Dependencies** 只读页：母版 revision（发布钉扎优先）/ 锚点 / 条款版本 / release 线；允许 release GET 加法 `masterPin`（U19-D5）后 FE Tab。Formal phase **None**；leave **#50**；**not** CD-3；**not** go-live；**not** CE-O02。Queued after: **#80** / **#77**。 |
