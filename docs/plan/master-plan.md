...
> **IBL (registered TM backlog):** [intl-bank-letter-readiness-program.md](./intl-bank-letter-readiness-program.md) — international bank letter readiness (`IBL-*`); Task Master **#106** (umbrella, registry only) + **#107–#134** (**28** leaves, all `pending`); program **Registered / Not Started** (waves **not** activated); formal phase **None**; Wave D residuals **D01A+#135+#136** closed (**#136** Done `a872c15b` / `8c52ee67`); host sole-active = **#137** PRR-P2 audit hygiene (**IBL vetoed** for this leaf); **IBL next queue head (not activated)** = **IBL-A1** (#107) or Wave A batch after #137. Merge `9fc2bc97`. Do **not** invent a P-phase. Do **not** flip checklist **#3b GO**. Do **not** activate IBL without delivery-orchestrator / explicit redirect.
