...
**Active formal phase / program:** **None** (2026-07-09+). **Delivery focus note (2026-07-11):** **CDP Wave CD-2** — **CD-E2E-T13 → In Progress** (slice `cdp-e2e-t13-materialize`; sole-active CDP E2E — package materialize on publish / default-route stability; BDD S1–S3 re-evidence + wave closeout); wave stays **In Progress** (partial; T01–T12 Done merges `1930842` / `6821f45` / `895f16e` / `3aed175` / `c62b1a1` / `1eb230b` / `55a6ab6` / `b16e52a` / `6e3f825` / `f12b193`; **only T13 In Progress** among CDP E2E). Formal phase remains **None** (do not promote CDP). Do **not** mark CD-2 Done until T13 green + merge. Do **not** activate LR-C10. **Prior:** **CD-E2E-T12 → Done** (slice `cdp-e2e-t12-i18n-brands`; merge `f12b193`; zh-CN + REDBC/GREENBC dual-brand golden screenshots @1920; BDD-CDP-I18N-001/002). **Prior:** **CD-E2E-T11 → Done** (slice `cdp-e2e-t11-audit`; merge `6e3f825`; Audit admin query/filter + export; BDD-CDP-AUDIT-001/002). **Prior:** **CD-E2E-T10 → Done** (slice `cdp-e2e-t10-fidelity`; merge `b16e52a`; Fidelity «viewed» confirmation; BDD-CDP-FID-001…004). **Prior:** **CD-E2E-T09 → Done** (slice `cdp-e2e-t09-compare`; merge `55a6ab6`; Preview vs final comparison; BDD-CDP-CMP-001). **Prior:** **CD-E2E-T07 → Done** (slice `cdp-e2e-t07-api-policy`; merge `1eb230b`). **Prior:** **CD-E2E-T08 → Done** (slice `cdp-e2e-t08-preview`; merge `c62b1a1`). **Prior:** **CD-E2E-T06 → Done** (slice `cdp-e2e-t06-master`; merge `3aed175`). **Prior:** **CD-E2E-T05 → Done** (slice `cdp-e2e-t05-publish`; merge `895f16e`). **Prior:** **LR-C9 → Done** (`lrp-c9-load-error-panel`; merge `0013615`); no LRP slice currently `In Progress`; [LRP-C detail](./detail/LRP-C-usability-deepening.md); [launch-readiness-program.md](./launch-readiness-program.md). **Prior:** Wave LR-A **Done** (A1–A7; merge `cc9e5f6`; **ADR-0041 Accepted**; 0042/0043 remain Proposed; Word/XSD/LO24 residuals deferred). **CODE-QUALITY program Done** — CQ-01A…CQ-08; ArchUnit **11/11**; `mvn verify` **GREEN**; `pnpm` **894** tests + build **GREEN**. **CORE-FORTRESS program Done** — F1–F8 complete. **Prior:** **P23-DEMO-TYPOGRAPHY-LAYOUT-EXCELLENCE Done** (2026-07-08).
...
>
> **CDP (active delivery focus):** [competitiveness-deepening-program.md](./competitiveness-deepening-program.md) — Wave **CD-0 Done**; Wave **CD-2 In Progress** (**partial** — T01–T12 Done, merges `1930842` / `6821f45` / `895f16e` / `3aed175` / `c62b1a1` / `1eb230b` / `55a6ab6` / `b16e52a` / `6e3f825` / `f12b193`; **CD-E2E-T13 → In Progress** sole-active slice `cdp-e2e-t13-materialize`); task prefix **`CD-*`** only. Do **not** mark full CD-2 Done until T13 green + merge.
> **LRP:** [launch-readiness-program.md](./launch-readiness-program.md) — Waves LR-A…LR-E; task prefix **`LR-*`** only; **Wave LR-A Done** (2026-07-10; A1–A7; merge `cc9e5f6`); **LR-A5 Done** (`lrp-a5-adr-closeout`; **ADR-0041 Accepted**; 0042/0043 remain Proposed); Word/XSD/LO24 residuals deferred — do not invent Word numbers; **LR-C9 Done** (`lrp-c9-load-error-panel`; merge `0013615`); **LR-C1/C4 Done** via CORE-FORTRESS F7; Wave LR-C partial (C2/C3/C5/C7/C8/C10–C13 Not Started). Formal phase remains **None** — do not promote LRP/CDP to a formal phase row.
