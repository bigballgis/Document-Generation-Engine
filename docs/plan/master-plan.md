...
**Active formal phase / program:** **None** (2026-07-09+). **Delivery focus note (2026-07-10):** **CDP Wave CD-2** — **CD-E2E-T08 → In Progress** (slice `cdp-e2e-t08-preview`; preview success + artifact download; closes T13 manifest gap); wave stays **In Progress** (partial; T01–T06 Done merges `1930842` / `6821f45` / `895f16e` / `3aed175`; T07 + T09–T13 Not Started). Formal phase remains **None** (do not promote CDP). **Prior:** **CD-E2E-T06 → Done** (slice `cdp-e2e-t06-master`; merge `3aed175`). **Prior:** **CD-E2E-T05 → Done** (slice `cdp-e2e-t05-publish`; merge `895f16e`). **Prior:** **LR-C9 → Done** (`lrp-c9-load-error-panel`; merge `0013615`); no LRP slice currently `In Progress`; [LRP-C detail](./detail/LRP-C-usability-deepening.md); [launch-readiness-program.md](./launch-readiness-program.md). **Prior:** Wave LR-A **Done** (A1–A7; merge `cc9e5f6`; **ADR-0041 Accepted**; 0042/0043 remain Proposed; Word/XSD/LO24 residuals deferred). **CODE-QUALITY program Done** — CQ-01A…CQ-08; ArchUnit **11/11**; `mvn verify` **GREEN**; `pnpm` **894** tests + build **GREEN**. **CORE-FORTRESS program Done** — F1–F8 complete. **Prior:** **P23-DEMO-TYPOGRAPHY-LAYOUT-EXCELLENCE Done** (2026-07-08).
...
>
> **CDP (active delivery focus):** [competitiveness-deepening-program.md](./competitiveness-deepening-program.md) — Wave **CD-0 Done**; Wave **CD-2 In Progress** (**partial** — T01–T06 Done, merges `1930842` / `6821f45` / `895f16e` / `3aed175`; **CD-E2E-T08 → In Progress**; T07 + T09–T12 Not Started); task prefix **`CD-*`** only. Do **not** mark full CD-2 Done until remaining matrix green. **Sole active CDP slice:** `cdp-e2e-t08-preview` (T08).
