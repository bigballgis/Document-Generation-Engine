...
| 2026-07-08 | **P23-T12 Done** — publish orchestration: `deploy/publish-all-demos.ps1` + `demo-import-shared.ps1` (`Get-DemoPublishExternalIds`, `Get-DemoAllowedApiAdGroups`, token resolvers); **13** template registry; lifecycle publish + API policy + credential issuance to `.tmp/credentials/`; evidence summary `.tmp/evidence/all-demos-publish-summary.json`; `deploy/demo-shared/README.md` publish section; `DemoPublishOrchestrationContractTest` + `DemoPublishRegistry` (**8** tests); `DemoFullFlowCatalogSeeder.DEMO_FULL_FLOW_EXTERNAL_ID` public; taskmaster **#6** → `done` — **Wave 3 publish complete**; **next P23-T13** runtime generate E2E |
