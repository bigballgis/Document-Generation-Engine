...
| **P22-T08** | backend-engineer | **`deploy/demo-trade-lc/`** — `DEMO-TRADE-LC-NOTICE`, `DEMO-TRADE-GUARANTEE-NOTICE`; TRADE; `imageRef`/`sealRef`; attachment section | P22-T01, P22-T02, P22-T03 | Not Started |
| **P22-T09** | backend-engineer | **`deploy/demo-collection/`** — `DEMO-RATE-CHANGE-NOTICE`, `DEMO-OVERDUE-COLLECTION`; emphasis regulatory footer; collection disclaimer master | P22-T01, P22-T02 | Not Started |
| **P22-T10** | backend-engineer | **`deploy/demo-annual-review/`** — `DEMO-ANNUAL-REVIEW`, `DEMO-FACILITY-RENEWAL`; CORP; dual-page; condition/loop/list | P22-T01, P22-T02, P22-T03 | Not Started |
| **P22-T11** | backend-engineer | **`deploy/demo-wealth/`** — `DEMO-WEALTH-STATEMENT`; WEALTH; multi-table `tableComponent`, `imageRef`, footer totals | P22-T01, P22-T02 | Not Started |
| **P22-T12** | backend-engineer | **`deploy/demo-fol/` package contract alignment** — `pageNumberingProfile`, dual-page footer asset hooks, `masterLayoutVersion` bump, generator test asserts section fields + structure contract (§12) | P22-T03 | Not Started |
| **P22-T13** | backend-engineer | **`deploy/import-all-demos.ps1`** + `DemoCatalogSeeder` / `DemoFullFlowCatalogSeeder` integration — priority import order, `catalogMarker` idempotency, skip/update logging | P22-T05…T12, P22-T14 | Not Started |
| **P22-T14** | backend-engineer | **FOL executive upgrade** — ≥100 pages (`folPageTarget`), 40 anchors bound, rich bindings (list, emphasis, styleRef, tableComponent, contentModuleRef, condition, loop); executive test dataset; publish callable | P22-T01, P22-T02, P22-T03, P22-T12 | Not Started |
| **P22-T15** | backend-engineer | **Quality gates + regression tests** — POI field/run/table assertions; PDF text extraction page-number tests; fidelity warning tests (BDD-013/014); optional E2E smoke (≥1 journey per demo group); `mvn verify` + Docker import smoke | P22-T01…T14 | Not Started |
