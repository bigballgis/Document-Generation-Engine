...
**Gate status:** **GREEN** — `mvn -B -ntp -f backend/pom.xml verify` BUILD SUCCESS (2026-07-03); architecture **C1–C3 resolved**; demo packages T05–T11 scaffolded; T14 FOL executive scale **Not Started**.
...
| **P22-T13** | backend-engineer | **`deploy/import-all-demos.ps1`** + `DemoCatalogSeeder` / `DemoFullFlowCatalogSeeder` integration — priority import order, `catalogMarker` idempotency, skip/update logging | P22-T05…T12, P22-T14 | **In Progress** (orchestrator script + shared import helper; full API import per demo pending) |
...
| **P22-T15** | backend-engineer | **Quality gates + regression tests** — POI field/run/table assertions; PDF text extraction page-number tests; fidelity warning tests (BDD-013/014); optional E2E smoke (≥1 journey per demo group); `mvn verify` + Docker import smoke | P22-T01…T14 | **In Progress** (`mvn verify` green; BDD-005/006 + stamp-failure regression added; Docker/E2E smoke pending) |
