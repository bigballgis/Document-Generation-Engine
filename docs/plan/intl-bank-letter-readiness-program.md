...
**Task Master:** Umbrella **#106** (registry only, **`pending`**) + leaves **#107–#134** (28 tasks = A6+B7+C3+D5+E7). **#107–#117** IBL-A1–A6 + IBL-B1–B5 → **`done`**; **#118** IBL-B6 → **`in-progress`** (sole-active); **#119–#134** remain **`pending`** (B7 + Wave E descriptions start with **BLOCKED**). **Sole-active:** **#118** IBL-B6 (`ibl-b6-repro-freeze`). See § Task Master ID map.  
...
| [execution-sync-ledger.md](./execution-sync-ledger.md) | **#118** IBL-B6 → **in-progress** (sole-active `ibl-b6-repro-freeze`); **#117** IBL-B5 → **Done** (`1666312b` / `d7459405`+`fbb40429`); **#116** IBL-B4 → **Done** (`610eb0fa` / `d6b389d1`+`d2e8a1c9`); **#115** IBL-B3 → **Done** (`3710811a` / `c81054b0`+`e0102ddb`); **#114** IBL-B2 → **Done** (`29d022b6` / `3dd1aa60`+`36a9821c`); **#113** IBL-B1 → **Done** (`a33da272` / `44237c99`); Wave IBL-B → **In Progress** (B1–B5 Done; B6 In Progress; B7 Blocked); Wave IBL-A remains **Done** |
...
| IBL-B4 | **116** | **done** |
| IBL-B5 | **117** | **done** |
| IBL-B6 | **118** | **in-progress** (sole-active) |
...
| **F16** | No deterministic legal-reproducibility freeze (LO version + font set + content-hash PDF baselines) | ops/rendering docs gap → **freeze authored** ([ADR-0060](../adr/rendering-authoring/0060-legal-reproducibility-freeze.md) + [ops](../operations/legal-reproducibility-freeze.md); #118 still In Progress until leaf closeout) | **Medium** | **IBL-B6** |
...
3. Formal phase remains **None**. Task Master registration (#106–#134) is complete; **Wave IBL-A** → **Done** (A1–A6); Wave **IBL-B** → **In Progress** (**#113/#114/#115/#116/#117** IBL-B1–B5 **Done**; **#118** IBL-B6 **In Progress** sole-active; B7 Blocked) — do not invent a P-phase here.
4. Tasks marked **BDD: required** may not start implementation until `behavior-spec-author` publishes a `ready` spec.

**Current wave:** Wave **IBL-B** → **In Progress** (Wave **IBL-A** → **Done**; B1–B5 **Done**; B6 **In Progress** sole-active; B7 **Blocked**).

```text
Sole-active: #118 IBL-B6 (ibl-b6-repro-freeze; ISOLATED D:/working/DGE-ibl-b6-repro-freeze · feat/ibl-b6-repro-freeze; base e2f0a505)
Next queue (not activated): #119 IBL-B7 (Blocked — Word host) → IBL-C → IBL-D → IBL-E (pending decisions)
```
...
**Wave status:** **In Progress** (B1–B5 **Done**; B6 **In Progress** (#118 sole-active); B7 **Blocked**)  
...
| IBL-B6 | doc-keeper (+ rendering-engineer) | Deterministic legal-reproducibility freeze doc/ADR (LO version + font set + content-hash baselines) | — | not-applicable | F16 | **In Progress** |
...
| **IBL-B6** | ADR or ops freeze doc records LO version, font set, and content-hash baseline procedure; indexed from docs. Gates: docs review (architecture-reviewer as needed). **In Progress** (#118 sole-active) — freeze authored: **[ADR-0060 Accepted](../adr/rendering-authoring/0060-legal-reproducibility-freeze.md)** + ops [legal-reproducibility-freeze.md](../operations/legal-reproducibility-freeze.md) (LO record-at-cut + ADR-0041 fonts + SHA-256 content-hash procedure; indexed from `docs/README.md` / ADR index / golden-corpus README). **BDD `not-applicable`:** [ibl-b6-repro-freeze.md](../behavior/ibl-b6-repro-freeze.md)（ops/ADR freeze documentation for F16；**not** a user journey；**no Word/pixel baselines invented**；**`frontend_ui_in_scope=false`**；**not** go-live；do **not** flip #3b/#5a；**OUT B7**；**#118 stays in-progress** until arch/docs gate + leaf closeout — do **not** mark Done here）。 |
...
Task Master registration is **complete** (#106–#134; MAIN merge `9fc2bc97`). **Wave IBL-A** → **Done** — **IBL-A1…A6 / #107–#112** → **Done** (A6 tip `5cacff2a`). **Wave IBL-B** → **In Progress** — **#113** IBL-B1 → **Done** (`a33da272` / `44237c99`); **#114** IBL-B2 → **Done** (`29d022b6` / `3dd1aa60`+`36a9821c`); **#115** IBL-B3 → **Done** (`3710811a` / `c81054b0`+`e0102ddb`); **#116** IBL-B4 → **Done** (`610eb0fa` / `d6b389d1`+`d2e8a1c9`); **#117** IBL-B5 → **Done** (`1666312b` / `d7459405`+`fbb40429`); **#118** IBL-B6 → **in-progress** (sole-active); B7 **Blocked**. Formal phase remains **None**. Wave B progress does **not** flip checklist **#3b** / **#5a**, claim go-live, or mark IBL program Done.
...
**Next action:** **#118** IBL-B6 **sole-active** — freeze docs authored (**ADR-0060** + ops); hand off to **architecture-reviewer** (docs gate) then remaining pipeline — **keep #118 in-progress** until closeout. Wave **IBL-B** stays **In Progress** (B1–B5 **Done**; B6 **In Progress**; B7 **Blocked**). Wave **IBL-A** remains **Done**. Do **not** invent a formal P-phase. Do **not** invent Word baselines. Do **not** flip checklist **#3b GO** / **#5a GO**. Do **not** claim go-live or IBL program Done.
