...
**Task Master:** Umbrella **#106** (registry only, **`pending`**) + leaves **#107–#134** (28 tasks = A6+B7+C3+D5+E7). **#107–#115** IBL-A1–A6 + IBL-B1–B3 → **`done`**; **#116** → **`in-progress`** (sole-active); **#117–#134** remain **`pending`** (B7 + Wave E descriptions start with **BLOCKED**). **Sole-active:** **#116** IBL-B4 (`ibl-b4-long-clause-overflow`). See § Task Master ID map.  
...
| [execution-sync-ledger.md](./execution-sync-ledger.md) | **#116** IBL-B4 → **in-progress** sole-active (`ibl-b4-long-clause-overflow`); **#115** IBL-B3 → **Done** (`3710811a` / `c81054b0`+`e0102ddb`); **#114** IBL-B2 → **Done** (`29d022b6` / `3dd1aa60`+`36a9821c`); **#113** IBL-B1 → **Done** (`a33da272` / `44237c99`); Wave IBL-B → **In Progress** (B1+B2+B3 Done; B4 In Progress; B5–B6 pending; B7 Blocked); Wave IBL-A remains **Done** |
...
| IBL-B2 | **114** | **done** |
| IBL-B3 | **115** | **done** |
| IBL-B4 | **116** | **in-progress** (sole-active) |
| IBL-B5 | **117** | pending |
| IBL-B6 | **118** | pending |
...
3. Formal phase remains **None**. Task Master registration (#106–#134) is complete; **Wave IBL-A** → **Done** (A1–A6); Wave **IBL-B** → **In Progress** (**#113/#114/#115** IBL-B1–B3 **Done**; **#116** IBL-B4 **In Progress** sole-active) — do not invent a P-phase here.
4. Tasks marked **BDD: required** may not start implementation until `behavior-spec-author` publishes a `ready` spec.

**Current wave:** Wave **IBL-B** → **In Progress** (Wave **IBL-A** → **Done**; B1+B2+B3 **Done**; B4 **In Progress**; B5–B6 pending; B7 **Blocked**; **sole-active #116**).

```text
Sole-active: #116 IBL-B4 (ibl-b4-long-clause-overflow)
Next queue (not activated): #117–#118; B7 gated → IBL-C → IBL-D → IBL-E (pending decisions)
```
...
**Wave status:** **In Progress** (B1+B2+B3 **Done**; B4 **In Progress**; B5–B6 **Not Started**; B7 **Blocked**; sole-active **#116**)  
...
| IBL-B4 | rendering-engineer + doc-keeper | Long-clause overflow/truncation/page-break policy + activate golden theme `08` | — | **required** (blocked until stage 1) | F13 | **In Progress** |
...
| **IBL-B4** | Written overflow policy (truncate vs paginate vs fail-closed) confirmed in BDD; theme `08-long-clause-limits` activated with LO golden assertions. Gates: backend verify (+ deploy if required). **In Progress** sole-active — slice `ibl-b4-long-clause-overflow`; ISOLATED `D:/working/DGE-ibl-b4-long-clause-overflow` · `feat/ibl-b4-long-clause-overflow` (base `3de54cbc`). BDD **required** — **blocked until stage 1**. Batch **solo** (`member_task_ids: ["116"]`; vetoes **B5 seal**, **B7 Word**). Owners **rendering-engineer + doc-keeper**. **`frontend_ui_in_scope=false`** expected; **not** go-live；do **not** flip #3b/#5a；OUT B5/B7. |
...
**Next action:** **#116** IBL-B4 is **sole-active** (`ibl-b4-long-clause-overflow`). Wave **IBL-B** stays **In Progress** (B1+B2+B3 **Done**; B4 **In Progress**; B5–B6 pending; B7 **Blocked**). BDD **required** — hand off to **behavior-spec-author** (stage 1) before implementers. Do **not** invent a formal P-phase. Do **not** flip checklist **#3b GO** / **#5a GO**. Do **not** claim go-live or IBL program Done. Do **not** activate B5/B7 until next deliver.
