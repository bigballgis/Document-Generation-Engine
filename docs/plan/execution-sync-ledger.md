...
**Last synced:** 2026-07-04 (**SOR F06/F07 slice** — `cursor/sor-full-implementation-1385`; SOR-F07 Done; SOR-F06 partial; frontend Vitest **667**; see [system-optimization-review-2026-07.md](./system-optimization-review-2026-07.md))
**Authoritative gate snapshot (2026-07-04):** backend Surefire **723** (1 skipped); frontend Vitest **667**; Playwright docker smoke tier **8** specs.
**Completion note (2026-07-04, SOR-F06/F07):** **SOR-F07 Done** — split `UserManagementPanel` into `UserManagementListSection`, `UserFormDialog`, `UserResetPasswordDialog`, shared `userPasswordRules`. **SOR-F06 In Progress** — identity `fetchUsers` abort signal + `lastUserErrorRetryable` + `LoadErrorPanel` on user list. **Gate:** `pnpm -C frontend lint`, `type-check`, `test` (**667**), `build` ✓.
