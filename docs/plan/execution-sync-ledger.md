...
**Last synced:** 2026-07-04 (**SOR wave 3/7 slice** — `cursor/sor-full-implementation-1385`; SOR-P03/T02/T03 Done; SOR-P02 size-cap slice In Progress; backend `mvn verify` **723** Surefire; frontend Vitest **656**; docker Playwright subset **8** specs; see [system-optimization-review-2026-07.md](./system-optimization-review-2026-07.md))
**Authoritative gate snapshot (2026-07-04):** backend Surefire **723** (1 skipped); frontend Vitest **656**; Playwright docker smoke tier **8** specs in CI (`test:e2e:docker`: catalog, role-journeys, master-replace-docx, content-module-lifecycle, collaboration-todos, a11y-smoke, sor-a11y-axe, sor-journey-gaps).
**Completion note (2026-07-04, SOR-P02/P03/T02/T03):** **SOR-P03 Done** — PDF conversion pool queue default 0, fail-fast `PDF_CONVERSION_CAPACITY_EXCEEDED` (503 retryable), Micrometer `docgen.pdf.conversion.pool.*`. **SOR-P02 In Progress** — `GeneratedArtifactSizeGuard` + `max-generated-artifact-bytes` (50 MiB) on `DocumentArtifactPipeline`; full stream/spool deferred. **SOR-T02 Done** — `e2e/sor-a11y-axe.spec.ts` (`@axe-core/playwright` WCAG2/2.1 AA + dialog focus trap). **SOR-T03 Done** — `e2e/sor-journey-gaps.spec.ts` (forbidden content, identity user/group admin, reset-password dialog). **Gate:** `mvn -B -ntp -f backend/pom.xml verify` BUILD SUCCESS; `pnpm -C frontend lint`, `type-check`, `test` (**656**), `build` ✓.
