...
**Last synced:** 2026-07-04 (**SOR F06 catalog slice** — `cursor/sor-full-implementation-1385`; SOR-F06 Done; frontend Vitest **674**; see [system-optimization-review-2026-07.md](./system-optimization-review-2026-07.md))
**Authoritative gate snapshot (2026-07-04):** backend Surefire **723** (1 skipped); frontend Vitest **674**; Playwright docker smoke tier **8** specs.
**Completion note (2026-07-04, SOR-F06 catalog):** **SOR-F06 Done** — `storeRequestSupport` + `useAbortableCatalogLoader`; catalog lists (users/masters/templates/content-modules/audit) route-scoped abort + `LoadErrorPanel` retryable UX; shared `lastListErrorRetryable` on list stores. **Gate:** `pnpm -C frontend lint`, `type-check`, `test` (**674**), `build` ✓.
