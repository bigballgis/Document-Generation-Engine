...
**Last synced:** 2026-06-30 (P21-X05 Done — UI quality & a11y fixes AUD-Q01..Q03)
**Completion note (2026-06-30):** **P21-X05 Done** — UI quality & a11y fixes (AUD-Q01..Q03): define/alias `--color-primary` for table keyboard focus ring; `:focus-visible` on nav items + breadcrumb links; design-token cleanup (bare hex/px → tokens); brand wordmark shows bank display name via i18n (`branding.*`) not internal `REDBC`/`GREENBC` codes. **Deliverables:** `theme/tokens.ts` (+ `tokens.test.ts`), `styles/global.scss`, `AppDataTable.vue`, `ManagementShell.vue` (+ test), `AppBreadcrumb.vue` (+ test), `BrandLogo.vue` (+ test), `TemplateDetailLifecycleTab.vue`; i18n `en.ts` / `zh-CN.ts`. **Gate:** `pnpm -C frontend lint`, `type-check`, `test`, `build` green (**508** Vitest). Backend unchanged. **Audit:** AUD-Q01/Q02/Q03 **resolved**. P21 phase **wrap-up** — next **P21-X01/X06** per §11.5. Detail: [detail/P21-role-journey-frontend-redesign.md](./detail/P21-role-journey-frontend-redesign.md).
...
| --- | --- | --- | --- |
| Frontend (latest full gates) | `pnpm -C frontend lint`, `type-check`, `test`, `build` | Green — **498** Vitest tests (2026-06-30) | P21-T11 audit-admin journey + AuditConsole view-only + business columns + event-type labels |
| Backend (latest full verify) | `mvn -B -ntp -f backend/pom.xml verify` | Green — BUILD SUCCESS (2026-06-30) | P21-T07 approval-path collaboration closure — Surefire all pass, JaCoCo met, Checkstyle/PMD/SpotBugs **0** (+6 writer/lifecycle tests). Prior: P21-T02 (2026-06-29) |
...
| P21 | In Progress (2026-06-29) | Role-journey frontend redesign — **P21-X05 Done** (2026-06-30; UI/a11y AUD-Q01..Q03; `--color-primary`, focus-visible, brand wordmark i18n, token cleanup; frontend **508** Vitest); **P21-X04 Done** (2026-06-30; backend capability + route + OpenAPI; AUD-P02/P05 backend + AUD-P06 + AUD-C05 resolved; `mvn verify` **553**, frontend **504** Vitest); **P21-X03 Done** (2026-06-30; permission fail-closed + unified `canAccessRoute`); **all four role clusters complete** — phase **wrap-up**; next **P21-X01/X06** — see [P21 detail](./detail/P21-role-journey-frontend-redesign.md) |

**Active phase:** **P21** (activated 2026-06-29; **clusters ①→④ Done**; **P21-X03 Done**; **P21-X04 Done**; **P21-X05 Done**; phase **wrap-up**; next slice
**P21-X01/X06**). All other formal phases P0–P11, P13–P20 **Done** except P12
