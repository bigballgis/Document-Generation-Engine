...
| **[PRR-B02 — AdGroupResolver SPI / LDAP CONDITIONAL](./behavior/prod-ldap-ad-group-resolver-spi.md)** | **ready** (2026-07-18) · Task Master **#105** · **sole-active** (`prod-ldap-ad-group-resolver-spi`) | **BDD-PRR-B02-LDAP-SPI** — `AdGroupResolver` SPI 契约 + 消费者 DI 解耦；LAB `ConfigAdGroupResolver` ≠ 生产 AD；声称生产无真实目录适配器 → fail-closed；**禁止**虚构公司目录坐标；checklist **#5a** 保持 **CONDITIONAL**（**不**标 GO）；**`frontend_ui_in_scope=false`**；formal phase **None**; **not** go-live；do **not** flip **#3b**；next after → **#103** Wave C |
| **[PRR-B02 Wave B — AdGroupResolver SPI](./behavior/prod-ldap-ad-group-resolver-spi.md)** | **ready** (2026-07-18) · Task Master **#105** · slice `prod-ldap-ad-group-resolver-spi` | **BDD-PRR-B02-LDAP-SPI-001…007** — 确认 `AdGroupResolver` SPI；`ApiCredentialAuthenticationFilter` 依赖接口（非 `ConfigAdGroupResolver`）；LAB/config stub 非生产；声称生产无真实目录适配器 → fail-closed；公司坐标 **UNKNOWN**；checklist **#5a** 保持 **CONDITIONAL**（**not GO**）；**`frontend_ui_in_scope=false`**；formal phase **None**; **not** go-live；do **not** invent LDAP/AD hostnames；do **not** flip **#3b** |
