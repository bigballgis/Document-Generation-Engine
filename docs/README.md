...
| **[PRR-D01c — Dashboard summary API](./behavior/prod-dashboard-summary-api.md)** | **ready** (2026-07-18) · Task Master **#136** · slice **Done** (`a872c15b` / feature `8c52ee67`) | **BDD-PRR-D01C-*** — `GET /api/management/v1/dashboard/summary` 授权组分桶计数；Overview 首屏停用 fetch-all；OpenAPI `getDashboardSummary`；**`frontend_ui_in_scope=true`**；E2E **3 passed** + UIUX **PASS** Critical=0；BE/FE gates **GREEN**；arch **PASS_WITH_NOTES**；**DEPLOY_OK**；formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO**；**sole-active superseded by #137**；IBL stays pending behind #137 |
| **[PRR-P2 — audit hygiene (dead-code / folder / Knip)](./behavior/prod-audit-p2-hygiene.md)** | **not-applicable** (2026-07-18) · Task Master **#137** · slice **in-progress** (sole-active; **not Done yet**) | Delete unused `TemplateService.listAll` (+ CatalogSupport); move `components/template/*` → `components/templates/` + imports; knip high-confidence unused-export cleanup only；**not** go-live；do **not** flip **#3b GO** / **#5a GO**；do **not** activate IBL；formal phase **None** |
...
| [PRR-P2 audit hygiene FE](./evidence/prod-audit-p2-hygiene/README.md) | Task **#137** — `components/template`→`templates` merge + high-confidence Knip unused-export cleanup (2026-07-18; in-progress) |
