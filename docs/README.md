...
| **[CE-K06 rendering fidelity (K06a)](./behavior/ce-k06-rendering-fidelity.md)** | **ready** (2026-07-15) · Task Master **#62** · Wave 1a | **BDD-CE-K06a-001…006** — `repeatHeaderAcrossPages` → `<w:tblHeader/>`; golden `02-cross-page-table` → ACTIVE; K06b/K06c deferred residual; formal phase **None**; **not** go-live |
...
| **[CE-C04 credential expires_at persist + expose](./behavior/ce-c04-credential-expires.md)** | **ready** (2026-07-15) · Task Master **#69** · slice `ce-c04-credential-expires` · **In Progress** | **BDD-CE-C04-001…012** — Flyway `expires_at` + issue/rotate write; end LifecycleSupport transitional derivation; `RuntimeCredentialSummaryView.expiresAt` / `EXPIRING_SOON`; auth expired/revoked codes; optional CallableVersion `deprecated`/`sunsetAt` display; FE E2E N/A; formal phase **None**; **not** Done; **not** go-live |
