...
| **[PRR-D01a — ResilienceFailureMapper + PDF pool](./behavior/prod-ops-resilience-pdf-pool.md)** | **ready** (2026-07-18) · Task Master **#104** · slice **Done** (`f1f79d14`) | **BDD-PRR-D01A-*** — ResilienceFailureMapper → GENERATION_SERVICE_UNAVAILABLE (503) / GENERATION_TIMEOUT (504)（非 TEMPLATE_VALIDATION_FAILED）；PDF 池有界 + 饱和 fail-closed（PDF_CONVERSION_CAPACITY_EXCEEDED）；Wave D **SPLIT** leaf；**not** full Wave D bag；**`frontend_ui_in_scope=false`**；`mvn verify` **1975** GREEN；arch **PASS_WITH_NOTES**；**DEPLOY_OK**；E2E/UIUX N/A；formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO**；next queue → **#135** D01B (**Done**) → dashboard summary API |
| **[PRR-D01b — actuator / nginx CSP / ADR-0044 honesty / IRC mapper](./behavior/prod-ops-security-hardening.md)** | **ready** (2026-07-18) · Task Master **#135** · slice **Done** (`6e776232` / feature `1ada6b41`) | **BDD-PRR-D01B-*** — metrics/prometheus Basic scrape（禁匿名；匿名 **401**）；nginx CSP/标准安全头；ADR-0044 单副本 / sticky SSE / 进程内限流诚实残差；`FailedSyncInvocationErrorMapper` IRC；dashboard FE **OUT**（下一叶）；**`frontend_ui_in_scope=false`**；`mvn verify` **1988** GREEN；arch **PASS_WITH_NOTES**；**DEPLOY_OK**；E2E/UIUX N/A；formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO**；next queue → dashboard summary API |
| **[PRR-D01c — Dashboard summary API](./behavior/prod-dashboard-summary-api.md)** | **ready** (2026-07-18) · Task Master **#136** · BE stage on `feat/prod-dashboard-summary-api` | **BDD-PRR-D01C-*** — `GET /api/management/v1/dashboard/summary` 授权组分桶计数（停用首屏 fetch-all）；OpenAPI `getDashboardSummary`；**`frontend_ui_in_scope=true`**（FE/E2E 后续）；formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO** |
