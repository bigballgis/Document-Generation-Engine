...
| **[CE-K07 golden corpus skeleton](./behavior/ce-k07-golden-corpus-skeleton.md)** | **ready** (2026-07-14) · slice `ce-k07-golden-corpus-skeleton` | **BDD-CE-K07-001…019** — `golden-corpus/` ≥8 主题包骨架；ACTIVE 最小样本 `nested-clauses` + `encrypted-pdf`；DOCX 关键路径 + PDF 文本断言（禁像素）；接入 `mvn verify`；K01–K06/G02 后续充实 PLACEHOLDER |
