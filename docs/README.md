...
| **[CE-G04 Legal hold minimum](./behavior/ce-g04-legal-hold.md)** | **ready** (2026-07-16) · Task Master **#75** · **in-progress** (slice `ce-g04-legal-hold`) | **BDD-CE-G04-001…017** — hold 实体 `TEMPLATE_WINDOW`\|`INVOCATION_SET`；两路 retention 删除前豁免；GLOBAL_ADMIN 专属管理页/API；create/release 审计无敏感 payload；ADR-0040/0048 正文不改；formal phase **None**; **not** go-live; **not** CD-3 |
...
| **[Specialist Runtime Fallback](./behavior/specialist-runtime-fallback.md)** | **ready** (2026-07-16) · slice `orch-specialist-retry-only` | **BDD-ORCH-SPECIALIST-FALLBACK** SRF-01…10 — **retry first** (≤3) then **BLOCKED**; no auto GP; opt-in `允许降级`/`allow-gp-fallback`; skill [specialist-runtime-fallback](../.cursor/skills/specialist-runtime-fallback/SKILL.md); formal phase **None** |
