...
| **[International Bank Letter Readiness (IBL)](./plan/intl-bank-letter-readiness-program.md)** | **Wave IBL-A In Progress** — TM **#106** registry-only pending + **#107** IBL-A1 **Done** (`f0a2b6fe` / `4bda5f2d`) + **#108** IBL-A2 **In Progress** (sole-active) + **#109–#134** pending; A1 Done / A2 In Progress / A3–A6 Not Started; formal phase **None**; do **not** mark Wave A / program Done |
...
| **[IBL-A1 — runtime fail-closed variable validation](./behavior/ibl-a1-variable-validation.md)** | **ready** (2026-07-18) · Task Master **#107** · slice **Done** (`f0a2b6fe` / feature `4bda5f2d`) | **BDD-IBL-A1-001…008** — generate missing required / wrong type / wrong enum → **422** `VARIABLE_VALIDATION_FAILED` + `fieldErrors`；无静默 blank；preview **aligned**；publish **scoped**（不校验 generate body）；**`frontend_ui_in_scope=false`**；`mvn verify` **2014** GREEN；arch **PASS_WITH_NOTES**；**DEPLOY_OK**；E2E/UIUX N/A；residuals R1 regenerate unhooked / R3 IRC no fieldErrors；Wave IBL-A stays **In Progress**；succeeded by sole-active **#108** IBL-A2；formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO** |
| **[IBL-A2 — ISO-currency FORMAT_AMOUNT](./behavior/ibl-a2-format-amount-currency.md)** | **ready** (2026-07-18) · Task Master **#108** · slice **In Progress** (sole-active) | **BDD-IBL-A2-001…010** — `FORMAT_AMOUNT(value, iso4217)` + `context.locale`：EUR+en-US ≠ `$`；USD/CNY 交叉；一元 locale-default **兼容**；缺失/非法 currency → `VARIABLE_COMPUTE_FAILED`；OpenAPI/ADR-0056 文档化；**`frontend_ui_in_scope=false`**；Batch **solo**；ISOLATED `D:/working/DGE-ibl-a2-format-amount-currency` · `feat/ibl-a2-format-amount-currency`；Wave IBL-A **In Progress** (A1 Done; A2 In Progress); formal phase **None**; **not** go-live；do **not** flip **#3b GO** / **#5a GO**；**not** IBL-A3 |
