...
| **[PRR-A04 — library export streaming](./behavior/prod-library-export-streaming.md)** | **ready** (2026-07-17) · Task Master **#101** · slice **Done** (`5b705f56` / feature `834ca1a6`) | **BDD-PRR-A04-001…010** — `LibraryExportService` 临时文件/流式组装；禁止全库 ZIP/嵌套包字节常驻堆；**兼容** CE-E03 `template-library-export-v1-zip` 契约；**`frontend_ui_in_scope=false`**；`mvn verify` **1929** GREEN；Stage 10 **DEPLOY_OK**；arch **PASS_WITH_NOTES** Critical=0；E2E/UIUX N/A；next queue **#102** PRR-B01；formal phase **None**; **not** go-live；do **not** flip **#3b** |
| **[PRR-B01 Wave B item 5 — true production contract](./behavior/prod-true-prod-contract.md)** | **ready** (2026-07-18) · Task Master **#102** · slice `prod-true-prod-contract` | **BDD-PRR-B01-TPC-001…012** — `docker-compose.prod.yml` 声称生产默认（单 `prod` profile、`APP_ENVIRONMENT=prod`、强制 Kafka async、无 demo classpath / AD stub LAB 默认）；`ProductionAsyncTransportGuard` 对齐其它 Production*Guards；LAB 显式分离；LDAP/AD 适配器 **OOS CONDITIONAL**；**`frontend_ui_in_scope=false`**；formal phase **None**; **not** go-live；do **not** flip **#3b** / **#5a GO** |
