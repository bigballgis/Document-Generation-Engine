...
| **[CE-K08 clause legal metadata](./behavior/ce-k08-clause-legal-metadata.md)** | **ready** (2026-07-15) · Task Master **#63** · slice **In Progress** (BDD only; worktree `DGE-ce-k08-clause-legal-metadata`) | **BDD-CE-K08-LM-001…015** — version `jurisdiction`/`effectiveFrom`/`effectiveTo`/`legalReviewRef`; catalog filters; publish gate `CONTENT_MODULE_EFFECTIVE_EXPIRED`; formal phase **None**; **not** go-live |
