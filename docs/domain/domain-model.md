...
- **签章 `sealRef`（IBL-B5 / F14，2026-07-19）：** 授权区校验为 **几何包含**，非 boolean-only。根级 `authorizedSealAreas[]` 供给轴对齐矩形（单位 **pt**；页局部坐标，原点页左上，`+x` 右、`+y` 下；`pageIndex` 0-based）。`placement` 存在时强制：`authorizedAreaId` 解析目录 + `sealBox`（至少 `xPt`/`yPt`；缺省宽高 **48×48 pt**）完全落在区内（边重合允许）；区外 / 跨页 → `SEAL_OUTSIDE_AUTHORIZED_AREA`；未知区 → `SEAL_AUTHORIZED_AREA_UNKNOWN`；非法目录/重复 id → `SEAL_AUTHORIZED_AREA_INVALID`；非法/缺失 `sealBox` 或仅布尔遗留 placement → `SEAL_PLACEMENT_GEOMETRY_INVALID`。`withinAuthorizedArea` **DEPRECATED / 非权威**。无 `placement` → 跳过几何门（inline 兼容）。`applyScaling` → `SEAL_SCALING_NOT_ALLOWED` blocker（签章/QR 不适用图片缩放 warning）。规格：[ibl-b5-seal-geometry.md](../behavior/ibl-b5-seal-geometry.md)。本叶不交付绝对定位 DOCX 写出。
...
| `contentModuleRef` | 递归展开引用模块（IBL-E6：写路径维护 CM↔CM 嵌套边；深度≤8；环路 fail-closed） |
...
- **CE-K08：** 法务元数据仅草稿版本可写；`effectiveFrom`/`effectiveTo` 皆非空时须 `effectiveFrom <= effectiveTo`；空 `effectiveTo` 表示无到期。内容模块目录 list 可按法务字段筛选（catalog filter version = 优先最新 `APPROVED`+`ACTIVE`，否则最新版本）。模板发布时，若任一引用版本满足 `effectiveTo != null && utcNow.isAfter(effectiveTo)`，发布门禁 `CONTENT_MODULE_EFFECTIVE_EXPIRED` 硬阻断；已锁定已发布版本运行期不因事后过期失败。字段类型与 catalog 筛选不变。**IBL-E5 / ADR-0066 修正：** 未来 `effectiveFrom`（`effectiveFrom != null && utcNow.isBefore(effectiveFrom)`）由正交硬项 `CONTENT_MODULE_EFFECTIVE_NOT_STARTED` 阻断（取代 CE-K08「未来 effectiveFrom 不阻断」）。规格：[ce-k08-clause-legal-metadata.md](../behavior/ce-k08-clause-legal-metadata.md)；[ibl-e5-effectivefrom-bulk-repin.md](../behavior/ibl-e5-effectivefrom-bulk-repin.md)。
- **IBL-E5 / F27 residual（ADR-0066 Accepted）：** 发布门禁硬项 `CONTENT_MODULE_EFFECTIVE_NOT_STARTED` 与 `CONTENT_MODULE_EFFECTIVE_EXPIRED` 并存；`utcNow == effectiveFrom` → 未生效项 PASS；**不**引入模板 `SCHEDULED` / 延迟激活生命周期。组范围管理 API `POST /api/management/v1/content-module-references/bulk-repin`（必填 `dryRun`；`toSemanticVersion` xor `useLatestApproved`）对调用方 `authorTemplates` 可见的**草稿**钉扎做批量改钉；复用 upsertReference 校验；锁定/已发布 → `SKIPPED_LOCKED`；已在目标 → `SKIPPED_ALREADY_AT_TARGET`；审计 `CONTENT_MODULE_BULK_REPIN`（含 dry-run）。无新角色 / capability；`frontend_ui_in_scope=false`。规格：[ibl-e5-effectivefrom-bulk-repin.md](../behavior/ibl-e5-effectivefrom-bulk-repin.md)；[ADR-0066](../adr/template-lifecycle/0066-effectivefrom-publish-and-bulk-repin.md)。impl **Done**（`688f9e58` / `20ead1ce`）。
...
- **CE-G05 正文全文检索与 where-used（2026-07-17）：** 对 `content_module_version.content_structure_json` 维护 PostgreSQL `tsvector`（或 GENERATED 等价）+ GIN；写入/更新版本时同事务同步；配置 **`simple`**（中英混合信函，避免 english stemmer 误伤）。可检索文本从 JSON 抽取人类可读节点（须覆盖 `paragraph` / `text` / list item 文本；键名与纯结构标点可剥离）；抽取失败 → 该版本向量为空（不 500）；写版本成功则向量最终一致。目录 `searchMode=FULL_TEXT` 时对 **catalog filter version**（同 CE-K08：优先最新 `APPROVED`+`ACTIVE`，否则最新）正文向量匹配，与 CE-U20 `status` / legal filters **AND**；`NAME`（缺省）保持 LR-C5 ILIKE，不依赖 tsvector。where-used：授权范围内引用该模块的模板清单（只读；复用 `template_content_module_reference` 及发布锁定关系；**不**要求扫描全部 binding JSON；响应不含条款全文）。规格：[ce-g05-annual-review-fts.md](../behavior/ce-g05-annual-review-fts.md)。**IBL-E6 扩展：** 见下条嵌套图治理（深度 where-used）。
- **IBL-E6 / F28 条款嵌套模块图治理（ADR-0067 Accepted，2026-07-20）：** CM 版本结构中的 `contentModuleRef` 构成有向 CM↔CM 嵌套边；在结构**写入成功路径**同步维护可查询嵌套边投影（显式边表或等价投影；物理表名为实现细节）。嵌套深度 = 自被保存版本根沿嵌套边的最长简单路径边数；无边 → `0`；硬上限 **8**（同构 `ComputeDslLimits.MAX_NESTING_DEPTH` / `MAX_DEPENDENCY_DEPTH`）；`>8` → fail-closed（`CONTENT_MODULE_NESTING_DEPTH_EXCEEDED`）。自环/互环/间接环 → fail-closed（`CONTENT_MODULE_NESTING_CYCLE`；写路径拒绝 + 发布门禁 + 渲染展开不得死循环/静默截断）。嵌套目标 `referenceKey` 须解析到授权可见模块，否则 `CONTENT_MODULE_NESTING_TARGET_UNRESOLVED`。模板→CM 钉扎（`template_content_module_reference`）与 CM↔CM 嵌套边正交。where-used 扩展为直连 + 嵌套闭包命中（`referenceKind` = `DIRECT`\|`NESTED`，含 `nestingDepth` / 非敏感路径摘要）；权威 = 嵌套图 + 祖先钉扎，**不**扫 binding JSON。生命周期 impact 与深度 where-used **同闭包**。发布时钉扎闭包内每个嵌套 `referenceKey` 必须已钉扎且结构可解析，否则硬项 `CONTENT_MODULE_NESTING_UNPINNED`（与 `CONTENT_MODULE_REFERENCES` 正交）；闭包环/超深亦硬 FAIL。已发布锁定版本不因本叶回溯改写 pins。无新角色 / capability；`frontend_ui_in_scope=false`。规格：[ibl-e6-clause-nesting-governance.md](../behavior/ibl-e6-clause-nesting-governance.md)；[ADR-0067](../adr/template-lifecycle/0067-clause-nesting-module-graph-governance.md)。impl **Done**（#133；MAIN `dcc42c81` / tip `0e542c03` / stage10 `32b62136`）。
...
- 发布阻断项包括不支持节点、变量引用缺失、样式引用缺失、条款或内容模块引用缺失、**引用条款/内容模块版本已过 `effectiveTo`（CE-K08 / `CONTENT_MODULE_EFFECTIVE_EXPIRED`）**、**引用条款/内容模块版本 `effectiveFrom` 未到（IBL-E5 / `CONTENT_MODULE_EFFECTIVE_NOT_STARTED`）**、表格组件引用缺失、母版锚点引用缺失、编号断裂且影响条款语义、表格无法可靠渲染、签章/盖章位置异常和 PDF 转换失败。
...
- **催办时限设置 UI 放置（已确认，2026-07-21 / TM #153）：** 全局默认仅在管理壳 **系统设置** 全页编辑；组级覆盖仅在分组/团队表面的 **团队设置** 对话框编辑；Dashboard Overview **不**承载催办时限配置面板（协作待办队列仍可在 Dashboard）。API 与能力位不变。行为：[reminder-timing-settings-ia.md](../behavior/reminder-timing-settings-ia.md)。
...
- 版本级别支持停用。
- 同号再发布（same-release republish）会将先前同 `releaseVersion` 的 `PUBLISHED` 行置为 `STOPPED` 并清空其 `release_version`，且写入与显式停用一致的 `DEACTIVATE_VERSION` 生命周期审计；模板级状态仍按本次发布进入/保持 `PUBLISHED`。
...
- **CE-E01（2026-07-16）：** 导出包格式族：`template-export-bundle-v1-json`（默认兼容）与 `template-export-bundle-v2-json`（自包含）。v2 ZIP 载体固定条目 `template-export-bundle.json` + `artifacts/master.docx`；JSON 清单含 `masterPin`（消费 CE-K01 `master_revision_id`/`master_file_hash`，见 §2.11）、`clauseSnapshots`（条款正文/结构快照）、`renderProfile`、`assetKeyManifest`（默认仅键，无资产字节）。导入 `dryRun=true` 产出依赖预检报告且不写业务表；提交导入在 blocking 依赖未清时拒绝并保持零半残；通过时单事务物化缺失条款快照为草稿内容模块并落地模板 `DRAFT`，母版仍绑定目标环境已批准 `masterId`（E01 本片不从 DOCX 自动建母版）。权威行为：[ce-e01-export-bundle-v2.md](../behavior/ce-e01-export-bundle-v2.md)；契约：[contract-outline.md](../api/contract-outline.md) / [openapi-v1.yaml](../api/openapi-v1.yaml)。
- **CE-E03（2026-07-17）：** 全库导出格式 `template-library-export-v1-zip`：根 `library-export-manifest.json` 索引授权范围内导出合格模板；每个模板以嵌套 E01 v2 ZIP（`templates/{templateId}.zip`）自包含携带；根级 `masters/`、`clauses/` 为去重批量目录（不替代嵌套包自包含性）；聚合资产键清单默认不含二进制。全库导入非本片范围（目标侧继续单模板 E01 import）。权威行为：[ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md)。
- **SYS-NORM Wave 7 — UAT→PROD 晋级包（2026-07-21；ALGI 对齐 2026-07-22）：** opt-in `dependencyClosure=PROMOTION` 在 E01 v2 / E03 上叠加依赖闭合：ZIP 嵌入资产二进制（`artifacts/assets/{assetKey}`；全库根级可去重 `assets/`）+ 条款嵌套闭包（`clauseNestingGraph`，深度≤8）；**不得**要求 DocumentBrand/LegalEntity sidecar；**不得**携带 secrets。导入自动识别嵌入物；dry-run 零业务写入；提交落地模板 **DRAFT**（须生产侧重新测试→审批→发布）；可选物化 CE-E02 库资产时须写入**目标模板拥有组**作用域行（不得重新引入平台共享目录），物化母版状态仅 **DRAFT**（两阶段 P2 — **禁止**经 pack 跳过 APPROVED）。默认非晋级路径语义不得回退。权威行为：[sys-norm-promotion-pack.md](../behavior/sys-norm-promotion-pack.md)；资产组隔离：[asset-library-group-isolation.md](../behavior/asset-library-group-isolation.md) §10；决策复用 [ADR-0071](../adr/template-lifecycle/0071-retire-document-brand-legal-entity-surfaces.md) Decision 5。
- **CE-E02（2026-07-16）+ ALGI 组隔离（2026-07-22 / TM #154）：** **资产库目录（Asset library catalog）** — 组归属逻辑实体（非模板子资源；**已 supersede** 平台共享目录 E02-C12）：必填 `groupCode`（拥有业务组；v1 硬隔离，无跨组共享）、`assetKey`（逻辑绑定键；语法 `^[A-Za-z][A-Za-z0-9._-]{0,127}$`；模板 `imageRef`/`sealRef` **仍为裸** `assetKey`，**禁止**在绑定中嵌入 `groupCode/` 前缀）、`assetClass`（`IMAGE`\|`SEAL`\|`OTHER`）、`status`（`ACTIVE`\|`DISABLED`）、`contentType`、`sizeBytes`、`contentSha256`、`originalFileName`、`uploadedBy`、`uploadedAt`。自然身份 / 唯一约束 **`(groupCode, assetKey)`**（二者 trim；同键可存在于不同组）。持久化表 `library_asset`：`group_code` NOT NULL + unique `(group_code, asset_key)`（非 UUID 代理键；有意例外）。物理 MinIO 对象键 **组命名空间**：`{groupCode}/{assetKey}`（可另有既有 `.png`/`.jpg`/`.jpeg` 候选后缀）。上传写 namespaced 对象 + 目录行；停用标记 `DISABLED` 并删除该组下可解析 namespaced 键。模板解析：对模板拥有组 `G` 与引用 `K`，仅当目录存在 **ACTIVE** `(G, K)` 且 namespaced 对象存在时成功；他组 ACTIVE / 裸键 / 外组对象 **不得**满足解析（fail-closed，既有 not-found `messageKey` 家族）。迁移 **ALGI-M1**：既有无组语义行 → quarantine `group_code` + `DISABLED` + 清除遗留裸键；**禁止**静默 ACTIVE 指派到虚构 `PLATFORM` 或 `CORP`。权威行为：[asset-library-group-isolation.md](../behavior/asset-library-group-isolation.md)；CE-E02 修正：[ce-e02-asset-library.md](../behavior/ce-e02-asset-library.md) §15；契约：[contract-outline.md](../api/contract-outline.md) / [openapi-v1.yaml](../api/openapi-v1.yaml)；存储边界：[data-storage-view.md](../architecture/data-storage-view.md)。
