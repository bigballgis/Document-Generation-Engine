...
- **SYS-NORM Wave 7 — UAT→PROD 晋级包（2026-07-21）：** opt-in `dependencyClosure=PROMOTION` 在 E01 v2 / E03 上叠加依赖闭合：ZIP 嵌入资产二进制（`artifacts/assets/{assetKey}`；全库根级可去重 `assets/`）+ 条款嵌套闭包（`clauseNestingGraph`，深度≤8）；**不得**要求 DocumentBrand/LegalEntity sidecar；**不得**携带 secrets。导入自动识别嵌入物；dry-run 零业务写入；提交落地模板 **DRAFT**（须生产侧重新测试→审批→发布）；可选物化 CE-E02 库资产与缺失母版，物化母版状态仅 **DRAFT**（两阶段 P2 — **禁止**经 pack 跳过 APPROVED）。默认非晋级路径语义不得回退。权威行为：[sys-norm-promotion-pack.md](../behavior/sys-norm-promotion-pack.md)；决策复用 [ADR-0071](../adr/template-lifecycle/0071-retire-document-brand-legal-entity-surfaces.md) Decision 5。
- **CE-E02（2026-07-16）：** **资产库目录（Asset library catalog）** — 平台共享逻辑实体（非模板子资源）：`assetKey`（≡ MinIO 可解析对象键；语法 `^[A-Za-z][A-Za-z0-9._-]{0,127}$`；**禁止**强制 `library/` 等解析器未查找前缀）、`assetClass`（`IMAGE`\|`SEAL`\|`OTHER`）、`status`（`ACTIVE`\|`DISABLED`）、`contentType`、`sizeBytes`、`contentSha256`、`originalFileName`、`uploadedBy`、`uploadedAt`。持久化表 `library_asset` 以自然键 `asset_key` 为主键（非 UUID；与可解析对象键对齐的有意例外）。上传写对象+目录；停用标记 `DISABLED` 并删除可解析对象键，使 `StructuredContentImageResolver` 对 `imageRef`/`sealRef` 继续按既有协议 fail-closed（**不**改解析器签名/顺序/错误码）。权威行为：[ce-e02-asset-library.md](../behavior/ce-e02-asset-library.md)；契约：[contract-outline.md](../api/contract-outline.md) / [openapi-v1.yaml](../api/openapi-v1.yaml)；存储边界：[data-storage-view.md](../architecture/data-storage-view.md)。
