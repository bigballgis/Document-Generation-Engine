...
- **CE-E03（2026-07-17）：** 全库导出格式 `template-library-export-v1-zip`：根 `library-export-manifest.json` 索引授权范围内导出合格模板；每个模板以嵌套 E01 v2 ZIP（`templates/{templateId}.zip`）自包含携带；根级 `masters/`、`clauses/` 为去重批量目录（不替代嵌套包自包含性）；聚合资产键清单不含二进制。全库导入非本片范围（目标侧继续单模板 E01 import）。权威行为：[ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md)。
