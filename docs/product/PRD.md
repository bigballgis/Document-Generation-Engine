...
- **CE-E03（2026-07-17）：** 授权管理员/编排人员可一次导出授权范围内全部（或筛选后的）合格模板为全库 ZIP：根 manifest + 每个模板的 E01 v2 自包含包 + 去重母版/条款批量目录；资产仅列键。管理端全库导出 UI 与全库导入本片不交付（API-first）。行为规格：[ce-e03-full-library-export.md](../behavior/ce-e03-full-library-export.md)。
