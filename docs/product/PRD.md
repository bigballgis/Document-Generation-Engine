...
其中测试人员、审批人员和法务审阅人按分组配置；审计管理员用于查看审计记录。
...
非主责角色采用最小权限：测试人员只执行测试通过/不通过判定，审批人员只执行审批通过/不通过判定（含多级模式下的合规阶段），法务审阅人只执行多级模式下的法务阶段判定，API 调用方只调用已授权 API；除已确认职责外，不获得额外母版/模板编辑权限。
...
- 分组管理员只能分配运营类角色（母版设计人员、模板编排人员、测试人员、审批人员、法务审阅人），不得分配全局管理员、审计管理员或分组管理员（防提权）。
...
- **默认 `SINGLE_TRACK`：** 正常审批判定由审批人员执行；管理员可按权限矩阵正常判定。**可选 `LEGAL_THEN_COMPLIANCE`：** LEGAL 由法务审阅人（+管理员）判定，COMPLIANCE 由审批人员（+管理员）判定；不可跳级、不可由纯审批人员代批 LEGAL。
...
- 提交测试生成模板所属组测试人员角色队列待办；测试不通过或回到草稿生成提交人或模板编排人员整改待办；**`SINGLE_TRACK`** 提交审批生成审批人员角色队列待办；**`LEGAL_THEN_COMPLIANCE`** 提交审批生成法务审阅队列待办，LEGAL 通过后再生成合规/审批队列待办；任一层审批不通过或回到草稿生成提交人或模板编排人员整改待办；进入待发布后生成具备发布权限角色的发布待办。
