...
- **CE-E02（2026-07-16；Wave 5 角色修正）：** 管理端提供平台共享资产库（上传 / 列表 / 停用图片与签章资产）；键名约定固化且与渲染引用键一致；印章类（`SEAL`）上传需管理员（`GLOBAL_ADMIN` / `GROUP_ADMIN`，吸收原 `TEMPLATE_APPROVER` SEAL 特权）；`IMAGE`/`OTHER` 上传含文档作者；停用仅管理员；渲染侧 `StructuredContentImageResolver` 协议不变。行为规格：[ce-e02-asset-library.md](../behavior/ce-e02-asset-library.md)；契约：[openapi-v1.yaml](../api/openapi-v1.yaml) `/api/management/v1/library/assets`。
