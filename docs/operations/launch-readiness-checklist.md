...
| 5a | Seam — AD Group resolution (LDAP/AD adapter) | **NO-GO** | Ledger seams: config-file stub; BDD [ops-ad-group-stub-close.md](../behavior/ops-ad-group-stub-close.md) (**BDD-OPS-AD-GROUP-STUB-001**); [ADR-0054](../adr/authorization-security/0054-ad-group-resolver-production-boundary.md) Accepted (docs-first; pending merge + guard) | Production AD/LDAP adapter + tests not evidenced. Slice `ops-ad-group-stub-close` / Task Master **#46** In Progress — do **not** flip to CONDITIONAL until implement + green gates + post-task-doc-sync. Residual after stub refuse: real adapter when company directory available (coords **UNKNOWN** — do not invent). |
...
| Blocking **NO-GO** items | #3b ADR-0042/0043+Word; #5a AD Group stub; #5b paste↔binding (#9 `JWT_SECRET` **GO** — merge `587cd9a`; #10 Kafka path → **CONDITIONAL** — not a blocking NO-GO) |
| Accepted **CONDITIONAL** residuals (if any) | #10 Kafka: fail-closed `KAFKA_IMAGE` path remediated; operator must supply company-approved coords (no invent registry hostname); no company pull evidence → not GO |
