...
| 5b | Seam — Paste cleaning ↔ binding validation | **NO-GO** | Slice `ops-paste-binding-seam` (Task Master **#47** **in-progress**; sole-active); BDD [BDD-OPS-PASTE-BINDING-001](../behavior/ops-paste-binding-seam.md) **ready** S1–S6. **Docs-first (2026-07-12):** domain-model §2.6.7 + OpenAPI `pasteCleaningEvidence` aligned to **ADR-0019** (script / iframe / **object** / **absolute** → **BLOCKED**; residue on Accept; binding + PublishGate fail-closed; **wire** only). **CD-HARD-T05** **In Progress**. Ledger seam annotated In Progress. | **Verdict stays NO-GO until code + gate evidence.** Implementers: object/absolute must be BLOCKED (not REMOVED/WARNING); persist non-sensitive residue; `computeBindingStatus` + PublishGate fail-closed; user-visible checklist; clean rewrite clears residue. **post-task-doc-sync** flips → **GO** (wired — **not** edit-time-only ADR). Orthogonal to **LR-A4**. Clearing #5b alone ≠ go-live; overall **NO-GO** while #3b open. |
