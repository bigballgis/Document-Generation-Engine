...
. $Shared
Import-DemoPackage -DemoRoot $DemoRoot -ConfigPath (Join-Path $DemoRoot 'config/commitment-template-config.json') -BackendUrl $BackendUrl -PostgresContainer $PostgresContainer -SkipSql:$SkipSql -SkipApi:$SkipApi -SkipMasterRefresh:$SkipMasterRefresh
