...
SELECT 'a1000001-0001-4000-8000-000000000001', 'RETAIL-ACCOUNT-OPEN-STD', 'RETAIL', 'Retail Account Opening Standard Clause', 'Standard retail account opening terms and conditions summary', '[]', '10000003', '10000003'
...
SELECT 'a2000001-0001-4000-8000-000000000003', cm.id, '3.0.0', 'APPROVED', 'ACTIVE',
       '{"blocks":[
         ...
         {"type":"paragraph","text":"Governing law: The Account terms and this letter are governed by the laws of England and Wales. The courts of England and Wales have non-exclusive jurisdiction."},
         {"type":"paragraph","text":"Data protection: We process your personal data in accordance with UK GDPR and our Personal Banking Privacy Notice. You may request a copy of the data we hold about you by contacting our Data Protection Officer."},
         {"type":"paragraph","text":"Signature: Please retain this letter for your records. An authorised signatory of Meridian Retail Banking confirms the Account opening on the terms stated above."}
       ]}',
       'Retail account demo Wave A bank-grade import', '10000003', '10000007'
...
SET content_structure_json = '{"blocks":[
    ...
    change_description = 'Wave A refresh - retail account opening operative terms',
    updated_at = (NOW() AT TIME ZONE 'UTC'),
    updated_by = '10000007'
FROM content_module cm
WHERE v.module_id = cm.id AND cm.module_code = 'RETAIL-ACCOUNT-OPEN-STD' AND cm.deleted_at IS NULL AND v.semantic_version = '3.0.0';

INSERT INTO content_module (id, module_code, group_code, name, description, shared_group_codes_json, created_by, updated_by)
SELECT 'a1000001-0001-4000-8000-000000000002', 'RETAIL-ACCOUNT-BALANCE-STD', 'RETAIL', 'Retail Balance Confirmation Standard Clause', 'Standard retail balance confirmation regulatory notice', '[]', '10000003', '10000003'
WHERE NOT EXISTS (SELECT 1 FROM content_module WHERE module_code = 'RETAIL-ACCOUNT-BALANCE-STD' AND deleted_at IS NULL);

INSERT INTO content_module_version (id, module_id, semantic_version, review_state, lifecycle_state, content_structure_json, change_description, created_by, updated_by)
SELECT 'a2000001-0001-4000-8000-000000000002', cm.id, '3.0.0', 'APPROVED', 'ACTIVE',
       '{"blocks":[
         ...
         {"type":"paragraph","text":"This confirmation is issued solely for the customer named above and may not be relied upon by any third party without our prior written consent."},
         {"type":"paragraph","text":"Governing law: This confirmation is governed by the laws of England and Wales. For complaints unresolved by us, you may refer the matter to the Financial Ombudsman Service."}
       ]}',
       'Retail account demo Wave A bank-grade import', '10000003', '10000007'
...
SET content_structure_json = '{"blocks":[
    ...
    change_description = 'Wave A refresh - retail balance confirmation terms',
    updated_at = (NOW() AT TIME ZONE 'UTC'),
    updated_by = '10000007'
FROM content_module cm
WHERE v.module_id = cm.id AND cm.module_code = 'RETAIL-ACCOUNT-BALANCE-STD' AND cm.deleted_at IS NULL AND v.semantic_version = '3.0.0';
