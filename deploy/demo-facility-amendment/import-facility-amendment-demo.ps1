...
. $Shared
Import-DemoPackage -DemoRoot $DemoRoot -ConfigPath (Join-Path $DemoRoot 'config/facility-amendment-template-config.json') -BackendUrl $BackendUrl -PostgresContainer $PostgresContainer -SkipSql:$SkipSql -SkipApi:$SkipApi -SkipMasterRefresh:$SkipMasterRefresh
